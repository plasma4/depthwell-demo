...
- **Refine** (`refine.zig`) — what a non-terrain parent block does at the next depth. A bush states a PLAN for its 4x4 child region instead of filling it, so one bush does not become sixteen. A submerged plan cannot borrow support terrain into empty child corners.
