...
Worldgen is a pure function, so every cache here is only a memo. Throw them all away and the world comes back identical, just far slower. They exist because generation asks the same question again and again: a chunk needs a one-block halo to work out its edge flags, so every border block is computed twice; a parent block is shared by the 16 child blocks below it; a structure's box is re-derived by every neighbor that checks whether something bigger overlaps it. Caching whole chunks alone answers none of that, and the Worley and FBM passes underneath are not cheap enough to redo for fun.

Each bank sits in static WASM memory with a fixed budget, so nothing grows without bound or fragments the heap.

**What decides the shape of a bank is its access pattern, and nearly every pattern here is a sweep.** A sweep visits a rectangle of cells in order, then comes back and visits the same rectangle again: the generator walking a chunk, the halo walking its border, the renderer redrawing the window every frame. Total capacity is the wrong thing to look at for a sweep. What matters is whether two cells that are live _at the same time_ can land in the same slot.

That is why most banks are **direct-mapped and tiled** (`dw.utils.tileIndex()`): a cell's slot is its position wrapped into a power-of-two tile, never a hash. Two cells share a slot only when they sit a whole tile apart, so as long as the tile is bigger than the sweep, a sweep has _zero_ conflicts by construction, not by luck. A chunk and its halo simply cannot evict each other.
