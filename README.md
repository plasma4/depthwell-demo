...
Four flags select an optimization profile. They are mutually exclusive, and each one replaces `-Doptimize`:

| Flag         | Zig mode      | `wasm-opt` | DWARF | Use it for                                      |
| ------------ | ------------- | ---------- | ----- | ----------------------------------------------- |
| _(none)_     | `-Doptimize`  | no         | yes   | daily work                                      |
| `-Dog`       | `ReleaseSafe` | no         | yes   | stepping through optimized code in the debugger |
| `-Do1`       | `Debug`       | `-O1`      | no    | a quick, small build that keeps safety checks   |
| `-Do2`       | `ReleaseSafe` | `-O2`      | no    | speed with safety checks                        |
| `-Dwasm-opt` | `ReleaseFast` | `-O4`      | no    | production                                      |

Only `-Dog` keeps DWARF. Binaryen 131 cannot read the DWARF 5 that Zig 0.16 writes, so every profile that runs `wasm-opt` must remove it first. All profiles keep the names section, so stack traces still show function names.
