...
#### How a chunk comes to life (the procedural order)

Before the specifics, here's the fixed pipeline a chunk runs through. Every stage is a deterministic function of the seed and the chunk's position, so the same chunk regenerates identically whether it is streamed in for the first time, revisited from cache, or rebuilt on load. Later stages only ever _read_ what earlier ones produced, never the reverse, which is what keeps the whole thing order-independent across chunk borders:

1. **Base terrain.** Each cell samples moisture and density noise to pick its foundation block (a stone variation, lava stone, air for caves, water in pools). This is the "raw" world with no features yet.
2. **Ores and gems.** A second noise pass overlays veins onto foundation blocks, recording the stone it grew inside as the block's underlay so the ore reads as embedded.
3. **Structures.** Larger, terrain-gated features (chambers, pillars, geodes, trees) are placed by a prioritized, collision-resolved planner. A structure may overhang into neighboring chunks, and higher-priority kinds win contested cells, so this stage decides _whether and where_ a feature exists at all.
4. **Decorations.** Cheap, context-aware flourishes (mushrooms, flowers, vines, shrubs) that only need local support checks, not cross-feature collision — the lighter-weight tier below structures.
5. **Modifications.** Any player edits (or water-sim changes) recorded in the `ModificationStore` are replayed over the freshly generated chunk, overriding whatever generation produced. This is the only stage that isn't purely procedural, and it always has the last word.
6. **Derived passes.** Finally, edge flags, lighting, and waterlogging are computed from the settled block ids/neighbors. These are render/simulation state, recomputed rather than stored (see "The fractal modification buffer").

The three feature tiers are worth keeping straight because it's easy to reach for the wrong one: **structures** are the heavyweight planner (priority, collisions, overhang, terrain constraints); **decorations** are the lightweight tier (local support checks only, no cross-kind collision); and multi-tile things are just several ordinary sprites that _require_ each other via the neighbor-support rules (there is no dedicated multi-tile object type). The rest of this section expands each stage in order.
...
Variation is purely per-tile: it never spans blocks. Multi-tile features are instead built from _distinct_ sprites glued together by neighbor-support rules — the two-tile `moss_shrub` bushes, for instance, are a `moss_shrub1` + `moss_shrub1_right` pair held together by `SpriteProps.requires` (see "Decoration pass"), so breaking either half topples the other through the same cascade the rest of the world uses. There is no separate "multi-tile object" abstraction; a group is just sprites that require each other.
...
A `ModCell` holds just the only three fields that cannot be recovered by regenerating the chunk: `id`, `base_id`, and `hp`. Everything else (`seed`, `edge_flags`, `light`, waterlogging) is _derived_ and is rebuilt by `materializeChunk()`, which replays every authored cell over a freshly generated chunk and then reruns the flag pass. So a chunk the player mined 30 blocks out of costs ~210 bytes here, not 4KiB. See some definitions and more details:
