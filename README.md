...
- **Depth**: How "deep" the player is. Depth starts at $6$ (see `STARTING_ZOOM_TIMES` in `zig/startup.zig`). Each time you enter a portal, the world zooms in by $4\text{x}$, making terrain look 4 times larger, and the depth increases by 1.
    - **Depth Ascend/Descend**: ascend means increasing depth by 1 (things look larger, D increases), and descend means decreasing
...
```zig
/// A static 2x2 grid of seeds only updated during depth ascend or game startup.
...
That's why the code tries as hard as possible to only generate two chunks per frame (except on startup or depth ascend, as that will use different logic). By doing this, the code can easily extract these chunks from `ChunkCache` lazily when the player moves in a way that requires the `SimBuffer` to pull chunks near the edge.
