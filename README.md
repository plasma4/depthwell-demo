...
Depthwell is a procedural fractal mining incremental. How deep can you explore? A minimal demo release is planned for late 2026 or early 2027.
...
- Look for items with an indicator above them. A furnace smelts ore into bars. A "core" with a gray hovering orb upgrades your pickaxe. Both act as crafting stations.
- A campfire is not a crafting station. It only gives light.
...
- `zig build -Dgen-enums` builds and also regenerates `enums.ts`.
...
See `build.zig` for the other options. The Zig Language Server in VSCode or VSCodium helps a lot. Set it to "watch" mode. It then rebuilds the WASM for you and gives errors, highlighting, and "Go to Definition".
...
Git works by default. `.vscode/settings.json` controls whether diffs are shown.

For Jujutsu, run `jj git init` and `jj bookmark track main --remote=origin` after you clone. To build for release, run `chmod +x ./build.sh` and then `./build.sh`. To commit to main, run `chmod +x ./push.sh` and then `./push.sh`. The Windows equivalents are a direct translation.
...
Depthwell is Zig compiled to WASM plus a thin TypeScript host, and it renders with WebGPU. The web version is free and always gets updates. A native build on Mach Engine is planned. The internal viewport is 480x270 and scales with the display resolution. `zig/root.zig` exports every function the host can call, and it also lists almost every Zig file, so use it as the map.

The seed is 1 to 100 letters from `a-z`. `ChaCha12` and `BLAKE3` turn it into over `10^140` possible worlds. Depth and chunk counts are bound only by storage and RAM. Hot paths use `FastHash` instead, which is described below.
...
The camera and the player use integer subpixels. Entities use floating-point pixels. Generation and modifications work in blocks. To ask *where* something is, you use chunks.
...
Depth 13 is the **base depth**. It is the only depth built from noise instead of inherited from a parent, and the only one with a finite size. It is a closed square walled in by `edge_stone`.
...
> [!NOTE]
> At depth 32 or less, the quadrant is always 0 and the suffix alone says everything.

A worked example. At D=6 your horizontal path could be `[2, 3, 1, 0, 3, 2]`. The zoom factor is 4, so each step is one base-4 digit:

$$X = 2 \times 4^5 + 3 \times 4^4 + 1 \times 4^3 + 0 \times 4^2 + 3 \times 4^1 + 2 \times 4^0 = 2958$$

In binary that is the 2-bit run `10 11 01 00 11 10`, which fits the suffix.

#### Rebasing past 32 depths

Past 32 levels the suffix overflows. Each extra depth triggers a **rebase**. The player is re-centered inside the 64-bit range and the oldest 2 bits leave the suffix. They become 3-bit origin offsets (`left_cell_x` and `top_cell_y`) in the range 0 to 6, stored in the prefix stack. A `u64` holds 21 of these 3-bit steps, and the code finds one with `idx / 21` and `(idx % 21) * 3`.

The rebase picks the new center so that a player can travel as far as possible in any quadrant before the coordinate runs out. In practice that is quintillions of chunks. If the game ever asks for a chunk that no quadrant can name, it crashes on purpose rather than showing the wrong world.
...
3. **Structures.** Chambers, pillars, geodes, trees, and sand or clay deposits get placed by a planner that resolves collisions. This decides *whether and where* a structure exists.
...
6. **Derived passes.** Edge flags and waterlogging get recomputed from the settled block ids. Lighting runs last, right before the data goes to the GPU. None of this is stored, because all of it can be rebuilt.
...
`water` is a packed union, not a plain bitfield. A liquid block only needs to know whether more liquid sits directly above it. A solid block needs the whole picture of the water beside and below it, for the shader's surface fill. A plant will want moisture and growth state later. The block's `id` picks which view is live. All three views agree on bit 0, so any code can ask "is this cell submerged?" without knowing the kind.

`seed` is 28 cosmetic bits. The shader and `resolveVariant()` use it, and nothing else does. It is never saved.
...
- The island tag, which leaves sand or clay, costs a **second** warped Worley sample a few rows above the block. A block cannot answer "am I near a surface?" from its own density.
...
- **Anchor.** The box jitters to anywhere in the cell, overhang included. Drawing the origin from the cell interior instead would leave a blank band along every cell edge and make the spawn grid visible.
- **Seat.** The box slides down onto the terrain surface. Seating cannot be a rule. A rule tests a box that is already final, and seating is the step that decides where the box belongs.
...
The terrain rules are a small vocabulary, not per-structure code: `solid` and `empty` over a region, `level` for flat ground under a footprint, `encase` for "walled in by rock, to this degree", and `custom` as the escape hatch. Read the `Encase` and `Region` doc comments first.

Structures live in `zig/state/structures/` and use PascalCase file names, because each one acts more like a class than a struct. `structures/Example.zig` is a fully commented walkthrough.
...
The hard part of a fractal mining game is this: a hole mined at D must still be a hole at D+1, as a 4x4 empty region, and a 16x16 one at D+2.

`zig/state/ancestor.zig` and `zig/state/refine.zig` own the answer. To build a chunk at D, the generator walks up through the parents from D-1 toward H. At each level it asks the `ModificationStore` and the `AncestorCache` whether the parent block was modified. At H it stops asking about chunks and reads the `QuadCache` material grid instead.

**Materialize is not the same as generate.** `generateChunk()` is pure procedure. `materializeChunk()` is that plus every `mod_store` edit replayed plus a flag recompute. It is the only supported way to turn a store entry into a `Chunk`.

Once a parent block is known, `applyAncestorLogic()` decides what each of its 16 children holds.

- **Carve.** A solid parent does not become a solid 4x4 square. `carvesSlope()` deletes edge cells so the parent's silhouette reads as a slope. The center 2x2 **core** is never carved, so a descent always has floor to land on.
...
- **Refine.** A decoration is not terrain. Filling all 16 cells with a bush gives 16 bushes. So each such sprite states a *plan* for its region and `refineChild()` answers one cell of it. A plan is `single` (one copy, such as a chest or a portal landing), `scatter` (1 to a few copies along the anchor row), `chain` (a hanging vine, deduplicated and length-capped), or `stamp` (a hard-coded shape, such as the little tree a 2x1 shrub becomes). Every plan keeps at least one copy, so a decoration never disappears as the player descends.
- **Duplicate ore, but keep gems rare.** An inherited **ore** keeps a cell based on how much of the same ore surrounds its parent. `overlaySupport()` reads the parent's 3x3 neighborhood as a coarse density field and samples it per child cell, so a vein grows along itself and a lone nugget frays into a ragged clump. A parent buried in its own ore keeps every cell. An inherited **gem** keeps about `GEM_COPIES_MEAN` cells out of 16 instead, so a gem stays a find. Both kinds always keep one anchor cell inside the uncarvable core, so no deposit is ever wiped out by a descent.
- **Disperse.** Fresh ore also gets dispersed at the new depth, exactly as it does at the base depth.

`RefinedTag` is the other half. It is 11 bits inside a `Block` that record where the block came from once its own sprite no longer says so: a cell of a hanging chain and how far below the ceiling it sits, or the canopy of a stamped tree. A tag is re-derived on every generation and is never saved. A canopy tag also blocks ore for a few depths, so a tree does not sprout copper while it still reads as a tree.

### Storing modifications

To have a fractal *mining* game, the world has to remember player edits. That reduces to one question per chunk:
...
1. Read existing modifications for a rectangle of chunks, about 1000 reads per second. Some frames need 16 to 32 new chunks in the `SimBuffer`, and future camera features will need more.
2. Write a new modification at 60fps. The edge-flag update is the more likely bottleneck.
3. Increase the depth in under 3 seconds.
4. Keep heap fragmentation and allocation churn low.
5. Hold the whole state in RAM.

An entry is **sparse**. Instead of a 4 KiB `Chunk`, a `ModEntry` holds a `modified` bitmap with one bit per block, plus a packed `ModCell` array in ascending block-index order. A chunk with 30 mined blocks costs a few hundred bytes, not 4 KiB.
...
Generation is not cheap. Each block needs several FBM and Worley passes. So the engine generates only **two** chunks per frame, or four when the player moves fast. Startup and a depth change use different logic. Each frame it does this:
...
3. Put every finished chunk the renderer touches, and every chunk generated for the `SimBuffer`, into the `ChunkCache`. Reading a chunk *out* of the `SimBuffer` does not touch the cache.

This removes frame spikes. Without it, a single frame could owe the `SimBuffer` 16 chunks. It changes no logic: a player can still teleport trillions of chunks away, and the old chunks simply age out of the cache.
...
The budget is 4 chunks per frame, so one chunk must take about 4 ms. That gives $4\text{ ms} \times 4 \text{ chunks} \times 60 \text{ fps} \approx 1 \text{ second}$. The debug UI shows the numbers.

Single-core speed and thermal throttling both matter, so check the device, not only the math. The "mid-tier mobile" throttle in DevTools is a reasonable proxy. Mobile is not a target. Test in ReleaseSafe, ReleaseFast, and with `wasm-opt`. Depthwell currently has plenty of headroom.

### The cache layers

Worldgen is a pure function, so the same coordinate always gives the same block. Depthwell cannot generate a small world up front and be done. Even the base depth holds about `2^60` blocks. Purity is what makes all of the caching below legal.
...
That is why most banks are **direct-mapped and tiled** through `dw.utils.tileIndex()`. A cell's slot is its position wrapped into a power-of-two tile, never a hash. Two cells share a slot only when they sit a whole tile apart. So as long as the tile is bigger than the sweep, the sweep has **zero** conflicts by construction. A chunk and its halo cannot evict each other.

A hash gets this exactly backwards, and that cost real frames. A hash scatters neighbors into unrelated sets, so an n-way set that draws n+1 live cells misses on **every** access, forever, and the next identical sweep repeats it. More capacity makes such a set rarer but never removes one. A hash is right when the live keys are a real working set rather than a rectangle. Two banks are like that.
...
| `ancestor.ancestor_cache` | Whole materialized chunks at parent depths. This is what recursive generation reads. | 8-way with CLOCK eviction, indexed by *distance* from D. The two nearest depths get 128 slots, the rest get 8. About 2 MiB. | `clearCaches(true)` |
...
`struct_cache` matters because the same cell is re-derived constantly: by every block inside the footprint, and again by every later kind's collision scan. A cached box is one that would really be built, which is what lets a neighbor's scan trust it. Nothing reachable from the structure pass re-enters `chunk_ctx`, because terrain sampling reads base terrain, which is upstream. But `isBeaten()` **does** re-enter `struct_cache` for neighboring cells of its own kind, and it can evict the very slot it was called from. That is why no cache pointer is ever held across that scan.
...
WGSL has less browser support than WebGL 2, but it lets the engine manage GPU memory directly and it is the more modern standard. It is also far faster than `drawImage()` and can do much more.

Zig holds as much state as possible. Zig generates the data, pans it, converts it to `f32` so WebGPU hits no precision problems, and writes it into the scratch buffer. The shader reads from there.
...
Numbers go through `drawNumber`, which takes the same `lcha`. Because L and alpha multiply while C and H add, a white sprite works as a mask: give it a chroma and a hue and it takes that color exactly.

Watch the coordinate space. `addEntity` takes a viewport-pixel center and a pixel size. `addEntitySized` takes UV by default, and its `system` field selects the space.

### Sprite variation

A `Block.id` is only the **base** tile. `resolveVariant()` in `zig/types/variation.zig` resolves the tile that is actually drawn, once per visible block per frame, on the CPU after lighting and before upload. Most sprites resolve to themselves.
...
The shader takes the block's 28 seed bits together with its 4 `hp` bits as `seed0`. It runs `murmurmix32` three more times to get `seed1` through `seed3`, so every block on screen has four independent streams. The last two drive erosion and edge flags.
...
Ores and gems use a multi-texture mask to save atlas space. For a gem block the shader:

1. Samples the background stone from the block's world coordinates, which preserves the 2x2 tiling.
2. Computes a shifted UV for the gem from 8 seed bits, so the gem lands at any of 256 subpixel offsets inside the block.
3. Samples a gem mask and mixes stone and gem by the mask's red channel.
4. Flips the mask horizontally or vertically at random, so two gems at the same offset still differ.
...
Water is a cellular automaton in `zig/state/water.zig`, run over the loaded `SimBuffer`. There is no separate water grid. A cell's volume, 0 to 15, lives in the block's `hp`. On a solid block `hp` is mining progress instead, so the two never coexist.
...
1. `beginSnapshot()` writes the header and every small section, all captured at that instant. It opens the `mod_store` section and freezes a *plan*: the key and entries index of each modified chunk.
...
Copyright (c) 2026 Leo Zhang. All rights reserved. Distribution of any portion of the code or the raw assets without explicit written permission is strictly prohibited.
