...
Depthwell serializes the _entire_ game state into a single versioned, self-describing binary blob in `zig/state/save.zig`. Zig builds the blob in a persistent buffer; the actual atomic write to OPFS (and the per-frame budgeting that drives it) lives on the JS side. (This is pre-demo, so saves may break at any time on a core-logic change: only _backward_ compatibility is ever a goal, never forward.)

The layout is little-endian:

- magic `"DWSV"`, then a `VERSION` u32 (packed `a.b.c` from `parseVersion()`).
- a run of **sections**, each `tag u16 | section_version u16 | byte_len u64 | payload`. Tags (`sprite_table`, `header_core`, `quadcache`, `inventory`, `menus`, `tools`, `misc`, `mod_store`) are an enum whose numbers are _never_ renumbered or repurposed — new ones only ever append.
...
One global guard sits above all of this: `in_tick`. The simulation sets it while a tick is mid-flight, and both `exportAll()` and `beginSnapshot()` refuse to serialize while it is set — a half-applied tick is not a valid, resumable state, and a panic/trap that leaves it set deliberately blocks saving that torn state forever rather than persisting garbage.
