...
- **Chunk cache** (`world.chunk_cache`): finished chunks at the current depth, and the only one of these banks the renderer reads at all. It is what the camera falls back to whenever it outruns the `SimBuffer`, whether the player moved fast or simply zoomed out. Tiled, with the tile sized from the widest window the camera can ask for: the visible grid at `CAMERA_MIN_ZOOM`, plus the lighting margin, rounded up to a power of two. A dev build lands on 64 by 32 chunks, which is 2048 slots and 8 MiB. Since the tile covers the window, a still camera settles at zero misses and a panning one pays only for the row or column that just came into view.
