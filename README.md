...
2. **Ores and gems** get added to the terrain. A second noise pass overlays ore "veins" onto these terrain blocks.
   (Base IDs only appear for "foundation" or all solid blocks, excluding edge stone.)
...
4. **Decorations.** Cheap, context-aware flourishes (mushrooms, flowers, vines, shrubs) that only need to check adjacent blocks (whether there's a solid block above and below, for example).
5. **Modifications.** Any player edits (or water flowing changes) recorded in the `ModificationStore` are replayed over the freshly generated chunk, overriding whatever generation produced. This is the only stage that isn't purely procedural, and it always has the last word.
6. **Derived passes.** Finally, edge flags, lighting, and waterlogging are computed from the settled block ids/neighbors. These are render/simulation state, recomputed rather than stored.
