...
Only `-Dog`/`-Doptimize=Debug` (Debug by-default with no other optimization level flags) keeps DWARF.
...
When building for production with Vite (using `npm run build` instead of `npm run dev`), use `zig build -Dgen-enums -Do4` (with WASM optimizations from Binaryen).
