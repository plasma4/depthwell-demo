...
Depthwell is a web-only procedural fractal mining game about a quiet world that grows more detailed as you descend. The current focus is the first survival slice: forage fiber and loose stone, knap a hammerstone in the world, gather fuel, light a campfire, and make one tool upgrade.
...
- You can test the portal logic wherever you want by pressing M, turning on creative, and placing a portal. You can ascend through the green inverted portal after entering a deeper depth through the purple portal.
    - A fresh descent fixes the coordinate frame and inherited material for the new depth. Edits made at the current deepest depth can affect a later descent. After you leave a depth, later edits there stay local and do not change descendants that were already formed.
...
Use Jujutsu for version control in this project. Git still handles the remote transport and hooks. `.vscode/settings.json` controls whether diffs are shown.

Jujutsu keeps immutable local history, which makes recovery easy when an experiment goes wrong. Run `jj git init` and `jj bookmark track main --remote=origin` after you clone. To build for release, run `chmod +x ./build.sh` and then `./build.sh`. To commit to main, run `chmod +x ./push.sh` and then `./push.sh`. The Windows equivalents are a direct translation.
...
Using `ChaCha12`, `Blake3`, and a seed of 1 to 100 lowercase letters gives over `10^140` possible maps. Performance-sensitive areas use `FastHash`, described in the procedural-generation section.
...
Fresh coordinate frames are created only when the player descends into a new depth. Ascending reads the recorded frame back, and a retraced descent uses that same frame. A player can edit a shallower current depth, but those edits stay local and do not rename or rewrite descendants that were already formed.
...
Generation is expensive. Background streaming currently generates one chunk per logic tick. Portal previews use a separate budget, starting at four chunks per frame and increasing it when the preview deadline requires it. Startup and a depth change use different logic. The background streamer does this:
...
WGSL has less browser support than WebGL 2, but it lets the engine manage GPU memory directly and is the more modern standard. It also supports work that `drawImage()` cannot handle, such as the background effect.

Zig holds as much state as possible. It generates and pans the data, converts render values to `f32`, and writes them into the scratch buffer. The shader reads that data and draws it.
...
Use `drawNumber()` for numbers. It takes the same `lcha` values. Because L and alpha multiply while C and H add, a white sprite works as a mask. Giving it a chroma and hue selects that color. Particles, rectangles, and text use the same trick.
...
The background uses low-octave fractal Brownian motion instead of a static image. Three layers move 8x, 32x, and 64x slower than the camera. For every 64 pixels the player moves, the layers move 8, 2, and 1 pixels. Each layer has its own colors, and the effect uses RGB mixing rather than OKLAB.
...
Water is an experimental cellular automaton in `zig/state/water.zig`, run over the loaded `SimBuffer`. It is known to need replacement. There is no separate water grid. A cell's volume, 0 to 15, lives in the block's `hp`. On a solid block `hp` is mining progress instead, so the two never coexist.
