...
// It has no -O0, -Og, -O1, -O2 or -O3 flag, so the digit in -Do1, -Do2 and -Do4 gives the
// wasm-opt (Binaryen) level and not a Zig code generation level.
//
...
//
//   (none)             Debug mode, which the LLVM backend compiles at -O0. No wasm-opt, and DWARF is kept.
//   -Doptimize=MODE    Any Zig mode. ReleaseFast is LLVM -O3 with no safety checks, and runs no wasm-opt.
//   -Dwasm-opt         Adds a wasm-opt pass on top of -Doptimize (useful, say, for combining with ReleaseSafe).
//   -Dog               ReleaseSafe with no LTO and no wasm-opt, so DWARF stays for a debuggable optimized build.
//   -Do1               Debug mode plus a light wasm-opt -O1 pass (DWARF is dropped).
//   -Do2               ReleaseSafe plus a wasm-opt -O2 pass (DWARF is dropped).
//   -Do4               ReleaseFast plus the full aggressive wasm-opt -O4 pass (DWARF is dropped).
...
//
// -Dog, -Do1, -Do2 and -Do4 each set the mode themselves. Give at most one of them,
// and do not combine them with -Doptimize or with -Dwasm-opt.
...
pub fn build(b: *std.Build) void {
    ...
    const opt_o4 = b.option(bool, "o4", "Add aggressive pass of optimizations (wasm-opt from Binaryen), forcing optimization level to ReleaseFast") orelse false; // -Do4
    const wasm_opt = b.option(bool, "wasm-opt", "Run a wasm-opt pass on top of -Doptimize (DWARF is dropped)") orelse false; // -Dwasm-opt
    const opt_og = b.option(bool, "og", "Debuggable optimized build: ReleaseSafe, DWARF kept, no LTO and no wasm-opt") orelse false; // -Dog
    const opt_o1 = b.option(bool, "o1", "Debug code generation plus a light wasm-opt -O1 pass (DWARF is dropped)") orelse false; // -Do1
    const opt_o2 = b.option(bool, "o2", "ReleaseSafe plus a wasm-opt -O2 pass (DWARF is dropped)") orelse false; // -Do2
    ...
    // Precondition for the switch below: exactly one profile flag, or none at all.
    const profile_count = @as(u8, @intFromBool(opt_o4)) + @intFromBool(opt_og) + @intFromBool(opt_o1) + @intFromBool(opt_o2);
    if (profile_count > 1) @panic("Give only one of -Dog, -Do1, -Do2 and -Do4: they each set the optimization mode.");
    if (profile_count > 0 and wasm_opt) @panic("-Dwasm-opt applies to -Doptimize builds: -Dog, -Do1, -Do2 and -Do4 already select their own wasm-opt level.");

    const profile: Profile = if (opt_o4)
        .max
    else if (opt_o2)
        .o2
    else if (opt_o1)
    ...
    // even when a profile flag answers the question instead. Zig rejects an option that no one reads.
    const requested_optimize = b.standardOptimizeOption(.{});
    if (profile != .standard and b.user_input_options.contains("optimize")) {
        @panic("-Doptimize conflicts with -Dog, -Do1, -Do2 and -Do4: they each set the optimization mode.");
    }

    const optimize: std.builtin.OptimizeMode = switch (profile) {
        .standard => requested_optimize,
        .o1 => .Debug,
        .og, .o2 => .ReleaseSafe,
        .max => .ReleaseFast,
    };

    // The profile picks the wasm-opt level, or -Dwasm-opt derives one that matches the mode.
    // A null level means wasm-opt does not run at all, which is the only way DWARF survives.
    const binaryen_level: ?[]const u8 = profile.binaryenLevel() orelse if (wasm_opt) switch (optimize) {
        .Debug => "-O1",
        .ReleaseSmall => "-Os",
    ...
    if (build_native) {
        ...
        if (profile == .og) {
            // This profile keeps DWARF, because wasm-opt never sees this binary.
            exe.root_module.strip = false;
            exe.export_table = true;
        }
        exe.rdynamic = true;
        ...
        const wasm_bin: std.Build.LazyPath = if (binaryen_level) |level| blk: {
...
/// The optimization ladder that the profile flags select.
/// `standard` obeys `-Doptimize`; every other value sets the mode itself.
const Profile = enum {
    standard,
    og,
    o1,
    ...
    /// The `wasm-opt` level for this profile,
    /// or `null` when the profile itself selects no level.
    /// `standard` returns `null` because `-Doptimize` and `-Dwasm-opt` decide it instead,
    /// and `og` returns `null` because `wasm-opt` must never see that binary.
    fn binaryenLevel(self: Profile) ?[]const u8 {
        return switch (self) {
            .standard, .og => null,
            .o1 => "-O1",
            .o2 => "-O2",
