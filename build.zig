...
//
// Four profile flags select an optimization ladder. They are mutually exclusive, and each one
// replaces -Doptimize:
//   -Dog       ReleaseSafe, keeps DWARF, no LTO, no wasm-opt. The only profile you can step through.
//   -Do1       Debug code generation plus a wasm-opt -O1 cleanup. Fast to build, keeps safety checks.
//   -Do2       ReleaseSafe plus wasm-opt -O2.
//   -Dwasm-opt ReleaseFast plus the full aggressive wasm-opt -O4 pass.
//
// Only -Dog keeps DWARF. Binaryen 131 cannot read the DWARF 5 that Zig 0.16 writes: it stops with
// "TODO: DW_LNE_define_file" at every level, and also with no -g flag, so each profile that runs
// wasm-opt must give --strip-dwarf. All of them keep the names section for readable stack traces.
...
pub fn build(b: *std.Build) void {
    ...
    const profile_count = @as(u8, @intFromBool(wasm_opt)) + @intFromBool(opt_og) + @intFromBool(opt_o1) + @intFromBool(opt_o2);
    if (profile_count > 1) @panic("Give only one of -Dog, -Do1, -Do2 and -Dwasm-opt: they each set the optimization mode.");

    const profile: Profile = if (wasm_opt)
        .max
    else if (opt_o2)
        .o2
    else if (opt_o1)
    ...
    const optimize: std.builtin.OptimizeMode = switch (profile) {
        .standard => b.standardOptimizeOption(.{}),
        .o1 => .Debug,
        .og, .o2 => .ReleaseSafe,
        .max => .ReleaseFast,
    };
    ...
    if (build_native) {
        ...
        if (profile == .og) {
            // The one extra profile that keeps DWARF, because wasm-opt never sees this binary.
            exe.root_module.strip = false;
            exe.export_table = true;
        }
        exe.rdynamic = true;
        ...
        // wasm-opt reads the compiler output as a tracked input and writes a tracked output, so the
        // build cache skips the whole pass while that input is unchanged. It is not an in-place edit
        // of main.wasm any more, which also stops it from optimizing an already optimized file.
        const wasm_bin: std.Build.LazyPath = if (profile.binaryenLevel()) |level| blk: {
            ...
            optimize_wasm.addArgs(&.{
                ...
                // Not optional: Binaryen stops with a fatal error if it must read Zig's DWARF 5.
                "--strip-dwarf",
                "--strip-producers",
            });
            ...
            // Invariant: wasm-opt must accept every feature that code generation can emit, or it
            // rejects the module at validation. Both lists come from wasm_features above.
...
/// `standard` obeys `-Doptimize`;
/// every other value sets the mode itself.
const Profile = enum {
    standard,
    og,
    o1,
    ...
    /// or `null` when `wasm-opt` must not run at all.
    /// A profile that returns `null` is the only kind that keeps DWARF.
...
/// See the invariant at the call site:
/// `wasm-opt` rejects a module that uses a feature it did not get told about.
