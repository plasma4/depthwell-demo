...
//
//   (none/Debug)   Simple debug build (O0), no wasm-opt.
//   -Dog           -Og debug code generation by injecting Zig compilation.
//   -Do1           -O1 debug code generation by injecting Zig compilation.
//   -Do2           -O2 debug code generation by injecting Zig compilation.
//   -Dwasm-opt     Adds wasm-opt pass (useful, say, for combining with ReleaseSafe); wasm-opt strips DWARF.
//   ReleaseFast    Default Zig -O3, no safety checks, plus wasm-opt pass.
//   -Do4           ReleaseFast plus the full aggressive wasm-opt -O4 pass.
...
pub fn build(b: *std.Build) void {
    ...
    const wasm_opt = b.option(bool, "o4", "Add aggressive pass of optimizations (wasm-opt from Binaryen), forcing optimization level to ReleaseFast") orelse false; // -Do4
