    ...
    if (build_native) {
        ...
        if (wasm_opt) {
            ...
            optimize_wasm.addArgs(&.{
                ...
                // No --flatten/--rereloop here: they run after the -O4 pipeline, so nothing
                // coalesces the locals they introduce. generateChunk went from 36 locals to
                // 2945, and V8 sizes wasm frames by local count, so the ~30-deep ancestor
                // recursion blew the JS stack (RangeError). Also 15% larger output.
                "--enable-simd",
                "--enable-sign-ext",
                "--enable-tail-call",
                "--enable-bulk-memory",
