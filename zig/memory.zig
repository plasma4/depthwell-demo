...
pub const GameState = extern struct {
    ...
    /// Anchored particles hold a world point that the camera jump leaves behind,
    /// so the anchor has to move with the camera at ANY distance (see `particles.syncAnchor()`).
