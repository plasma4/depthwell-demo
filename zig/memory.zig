...
///
/// NOTE: updating this currently WILL result in all previous save data becoming invalid!
...
pub const GameState = extern struct {
    ...
    ///
    /// `seed` above is the derived 512-bit value and the mixing that produces it is bijective but not invertible here,
    /// so without this a save can generate its world but can never say which seed made it.
    ...
    ///
    /// New fields belong at the END of `GameState`: `src/enums.ts` carries generated byte offsets for `seed` and `seed2`
    /// that the host writes through directly, and inserting above them silently moves the target of those writes.
    ...
    pub inline fn teleport(self: *@This(), coord: ?Coordinate, new_position: Vec2i) void {
        ...
        if (coord) |c| {
            // `commitLayer()` passes null and installs the destination itself, so every coordinate that
            // reaches here names a chunk at the CURRENT depth and must be one the world has.
