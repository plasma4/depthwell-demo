...
pub const GameState = extern struct {
    ...
    ///
    /// `seed` above is the derived 512-bit value and the mixing that produces it is bijective but not
    /// invertible here, so without this a save can generate its world but can never say which seed
    /// made it. That is the difference between a reproducible bug report and an unreproducible one.
    /// Only the first `seed_string_len` bytes are meaningful; the rest stay zero.
    ///
    /// New fields belong at the END of `GameState`: `src/enums.ts` carries generated byte offsets for
    /// `seed` and `seed2` that the host writes through directly, and inserting above them silently
    /// moves the target of those writes.
    ...
    /// Records the seed string the world was created from. Truncates rather than rejecting an
    /// oversized one, since the derived `seed` is already in place by the time this is called and
    /// half a record beats none.
