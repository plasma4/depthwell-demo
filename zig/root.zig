...
/// Master switch for developer tooling: the debug UI in the corner and everything that exists only
/// to serve it. Tuning sliders, heatmaps, creative mode, the depth hotkeys, the audit and preview
/// passes, and the logger's text buffers all disappear when this is false.
///
/// DELIBERATELY independent of the build mode. It used to force `is_debug` on, which meant a
/// `ReleaseFast` build was still a debug build and there was no way to profile the code that ships.
/// Keep this on in `ReleaseFast` and you get the real generated code with the panel still attached;
/// turn it off for a production build.
