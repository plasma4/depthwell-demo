...
/// Whether the cell at (bx, by) supports a protected installation and so cannot be dug out without the
/// structure tool: an unmineable floor-anchored block resting on it from above, or an unmineable
/// ceiling-anchored one hanging from it below. Either way, breaking the support would cascade the
/// installation out (see `Sprite.supports()`), which the structure tool exists to gate.
