...
///
/// `block_at()` must return the same world state that contains `block`.
...
pub fn canBreakWithLookup(
    ...
    if (!has_structure_tool and restsOnProtectedInstallationWithLookup(context, block_at, coord, bx, by)) return false;
...
fn restsOnProtectedInstallation(coord: world.Coordinate, bx: u4, by: u4) bool {
    return restsOnProtectedInstallationWithLookup({}, liveBlockAt, coord, bx, by);
}
...
fn restsOnProtectedInstallationWithLookup(
    ...
    const above = if (by > 0)
        block_at(context, coord, bx, by - 1)
    else blk: {
        const above_coord = coord.moveY(-1) orelse return false;
        break :blk block_at(context, above_coord, bx, dw.CHUNK_SIZE - 1);
    };
    ...
    const below = if (by < dw.CHUNK_SIZE - 1)
        ...
        break :blk block_at(context, below_coord, bx, 0);
    };
