...
pub fn tickDroppedItems(logic_speed: f64) void {
    ...
    const Context = struct {
        fn update(frames: u16, item: *DroppedItem) void {
            ...
            // Each frame closes `factor` of the gap to the player, so `frames` of them leave
            // (1 - factor)^frames of it. One step of that compounded factor equals many small steps,
            // which is what keeps the flight the same length in seconds at any tick rate.
