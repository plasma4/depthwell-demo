...
///
/// A tile, NOT a hash of the coordinates. Two cells share a slot only when they sit a whole tile
/// apart on an axis, so any working set that fits inside one tile takes ZERO conflict misses:
/// generation walks a window a few chunks wide, which is exactly the case a hashed index handles
/// worst (nine cells scattered over 64 slots collide about half the time by the birthday bound).
///
...
pub inline fn tileIndex(comptime width: usize, comptime height: usize, x: anytype, y: anytype) usize {
    comptime {
