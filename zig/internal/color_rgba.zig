//! Colors: the `ColorRgba` struct and its tests.
...
/// Represents a color.
/// WebGPU works in `rgba16float`.
/// These bytes only compare blocks for similarity, so they are not color-space compliant.
///
