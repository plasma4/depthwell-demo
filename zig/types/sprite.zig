...
const CORE_ID = GEAR_ID + 17;

/// Index where inventory slot sprites start.
pub const INVENTORY_START = GEAR_ID + 37;
...
comptime {
    if (max_sprite_value != 267) @compileError("Max sprite value unexpected!");
}
...
pub const Sprite = enum(u16) {
    ...
    cornflower = GEAR_ID + 6,
    ceiling_flower = GEAR_ID + 7, // 2 variations
    mushroom = GEAR_ID + 9, // 3 variations
    big_mushroom = GEAR_ID + 12, // 3 variations
    forest_furnace = GEAR_ID + 15,
    lava_furnace,
    basic_core = CORE_ID,
    core1 = CORE_ID + 2,
    core2 = CORE_ID + 4,
...
const rules = [_]SpriteRule{
    ...
    // The vertical plant: a `cornflower` cap over a `plant_base` shaft standing on the floor. Each link pins
    // the one below it (like the shrub halves), so mining the base topples the whole plant as a unit.
    .{
        .{ .single = .plant_base },
