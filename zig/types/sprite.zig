...
const rules = [_]SpriteRule{
    ...
    .{
        .{ .single = .amethyst },
        .{ .strength = 65, .required_capabilities = .t2 },
    },
    .{
        .{ .single = .sapphire },
    ...
    // Moss spreads rather than converts: 40% of it is still plain moss one depth down, so a patch
    // mottles into the two kinds instead of flipping wholesale.
    .{
        .{ .single = .mossy_stone },
    ...
    // Molten pockets inside lava stone: a fifth of it melts, gathered into patches a few blocks across
    // rather than scattered cell by cell.
    .{
        .{ .single = .lava_stone },
...
pub const Evolution = struct {
    ...
    /// 0 rolls every cell on its own, which reads as static;
    /// a positive size keeps the same `chance` but arranges it as soft patches instead.
    blob: f32 = 0,
};
