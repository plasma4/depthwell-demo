...
//!
//! Contents are never stored: they derive purely from the world seed + the chest's block position
//! (`rollLoot()`), so an unopened chest costs no save space and rerolls identically forever.
//! Clicking any filled slot loots EVERYTHING at once; the chest block is then removed
//! (persisting via `mod_store`, so a looted chest stays gone) and bursts into particles.
...
const loot_table = [_]LootEntry{
    ...
    .{ .item = .leaves, .min = 3, .max = 8, .weight = 14 },
...
/// Forgets the open chest. Called when the menu toggles closed (the rolled contents are transient).
pub fn close() void {
    open_chest = null;
}
...
/// Deterministically fills `stacks` from the world seed + the chest's block position.
fn rollLoot(ref: dw.indicators.BlockRef) void {
    ...
    for (stacks[0..n]) |*stack| {
        ...
        stack.* = .{
            .item = entry.item,
            .count = entry.min + @as(u32, @intCast(rng.next() % (entry.max - entry.min + 1))),
        };
    }
}
