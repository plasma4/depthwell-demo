...
/// Queues `entity` on the tile whose center is at `center_px`, where one world pixel is `zoom`
/// viewport pixels and the layer's tiles are drawn at `opacity`.
///
/// Queued, not drawn: the tile pass owns the scratch buffer while it runs, and pushing an entity
/// into it there would land in the middle of the block grid.
/// `updateEntities()` drains the queue first, so these sit over both tile layers and under
/// everything else the entity pass draws.
///
/// The camera comes from the layer that queued it, so a portal transition places these correctly
/// with no special case here: `LayerPass` already carries the override and the preview depth.
...
///
/// The campfire is the reason this exists.
/// Its flame is a separate sprite from its base so the fire can be tinted by the fuel burning in it
/// while the pile of wood underneath stays the same.
...
pub inline fn blockOverlay(block: memory.Block, frame: u32) ?BlockEntity {
    return switch (block.id) {
        // Animated here rather than by a variation.zig rule, because the frames belong to the
        // entity now and the base tile no longer changes.
        .campfire_base => .{
