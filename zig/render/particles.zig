...
/// One logic tick covers `logic_speed` of these frames, so the same particle traces the same curve
/// per second whether the game ticks at 60 or at 15.
pub const Particle = struct {
    ...
    ///
    /// Fractional on purpose: a tick shorter or longer than one frame ages a particle by exactly
    /// what it covered, and `draw()` reads a value BETWEEN two ticks to fade it smoothly.
    frames_left: f32 = 0.0,
...
///
/// Exact for a constant acceleration in both directions, which is what makes `draw()` able to walk
/// BACKWARD through a tick: stepping forward by `dt` and then back by `dt` returns the start pose.
