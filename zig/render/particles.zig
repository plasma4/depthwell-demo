...
pub const Particle = struct {
    ...
    /// An anchored particle scrolls with the camera and doesn't scale with zoom; see `anchor_cam`.
...
///
/// A backstop, and not the guard itself: every jump the game makes on purpose calls
/// `syncAnchor()`, which leaves no gap to measure. This only catches a camera moved by
/// something that forgot to, where drawing the jump as a pan would fling every particle.
