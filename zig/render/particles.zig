...
pub const Particle = struct {
    ...
    /// An anchored particle scrolls with the camera; see `anchor_cam`.
    /// Spawn one from `anchorScreenPx()`, never from an interpolated render position.
    anchored: bool = false,
};

// ----
// Anchoring. A particle position is a viewport pixel, so a screen-space particle rides the camera:
// a puff of dust would follow a running player instead of being left behind on the ground.
// An anchored particle is stored against `anchor_cam` rather than against the live camera,
// and `tick()` slides it by however far the camera moved since the last tick.
// So one subtraction per live anchored particle inside a loop that already runs, and no extra pass.
// ----
...
/// and `syncAnchor()` re-seats it after a teleport.
...
/// Call after anything that moves the camera without panning it, such as a teleport.
...
///
/// A pan of `d` subpixels moves the world `-d` pixels across the screen, so an anchored
/// particle must move with it.
/// A jump too large to be a pan is a teleport: the anchor re-seats and nothing slides,
/// because the world the particles belonged to is gone anyway.
...
/// Anchored particles must spawn from this and not from an interpolated render position,
/// or they start out measured against a camera that `tick()` will not correct for.
...
///
/// `tick()` only catches the anchor up at tick boundaries, and it runs BEFORE the ticks it is
/// paid for, so at draw time the anchor is a whole `handleTick()` behind the camera.
/// Taking that gap plus the sub-frame interpolation back out is what keeps dust glued to
/// the ground at any frame rate.
fn anchorDrift() Vec2f32 {
    ...
    // `tick()` reads the same jump the same way; drawing it as a pan would fling every particle.
