...
/// Random seed used for particle spawning.
/// Seeded in `startup.init()`.
...
pub fn spawnOrbitRing(origin: Vec2f32, colors: []const Vec4f32, config: OrbitConfig) void {
    ...
    for (0..config.count) |_| {
        ...
        // Squared so the ring crowds toward its inner edge. Spread evenly, most of a
        // disc's area, and so most of its particles, lands at the rim and leaves the
        // mouth itself bare
