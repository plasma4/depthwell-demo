...
pub const Particle = struct {
    ...
    /// the acceleration at distance `d` is `pull / (d^2 + PULL_SOFTENING^2)`,
    /// in viewport pixels per render frame^2. 0 leaves the particle travelling in a straight line.
    ///
    /// Inverse square rather than a flat magnitude falloffs are what make the particles seem like a swirl,
    /// at least on the "macro-scale" end of things.
    /// Angular speed climbs as the mouth nears, so a particle visibly whips around it instead of coasting in at one rate.
...
pub const OrbitConfig = struct {
    ...
    /// which is the behavior the lifetime is really covering.
    travel_min: u16 = 14,
    travel_max: u16 = 26,
