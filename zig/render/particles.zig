...
pub const Particle = struct {
    ...
    /// The magnitude is constant so paths stay predictable however close they get.
    pull: f32 = 0.0,
...
pub fn draw() void {
    ...
    for (&pool) |*p| {
        ...
        // Render frames run ahead of logical ticks, so both the position and the rotation are carried
        // forward by that fraction of a tick; the size is fixed for a particle's whole life.
        const lead: f32 = @as(f32, @floatCast(dw.chunks.current_dt)) + 1.0;

        dw.entity.addEntity(.{
            .sprite = .particle,
            .position = p.position + p.velocity * @as(Vec2f32, @splat(lead)),
            .size = p.size,
