...
/// applies sprite variation/animation, and queues the entities blocks want drawn over themselves.
/// Runs AFTER lighting so the lighting pass sees base (unvaried) sprite IDs.
///
/// Both jobs need the same walk over the same cells, and the grid is the largest thing this
/// frame touches, so they share one pass rather than reading it twice.
///
