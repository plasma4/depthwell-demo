...
///
/// The background's noise field is sampled at a clock, so the clock is a position and takes
/// the position curve: `bg_time + bg_time_step * current_dt` walks back toward the previous value.
