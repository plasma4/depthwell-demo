...
/// `anim_frame` interpolated to THIS render frame. Every animated sprite must read this, not `game.frame`.
///
/// Takes the position curve (see `current_dt`), so six render frames of one 10 FPS tick show six
/// different animation frames instead of one held frame and then a jump.
