...
comptime {
    ...
    std.debug.assert((BG_WRAP_CHUNKS * dw.CHUNK_SIZE) % @as(u32, @intFromFloat(BG_CELL_PIXELS)) == 0);
}

/// World pixels along one edge of a background pixel.
/// 16.0 is one block, so the background is pixelated on the block grid
/// and one chunk holds 16 background pixels per edge.
pub const BG_CELL_PIXELS: f64 = 16.0;

/// Smallest a background cell may be on screen, in canvas pixels.
/// The background costs 4 noise evaluations per cell (one per corner of its quad) against the
/// 1 per canvas pixel it replaces, so a cell must stay well above 2x2 canvas pixels to be worth it.
/// Only a zoomed-out debug camera reaches this floor; player zoom never does.
const BG_MIN_CELL_CANVAS_PIXELS: f64 = 8.0;
...
fn publishBackgroundGrid(effective_zoom: f64, canvas_w: f64, canvas_h: f64) void {
    ...
    // keeps a cell a whole number of blocks and keeps the grid's phase: a coarser cell is exactly the
    // 2x2 of a finer one, so the pattern does not slide sideways as the camera zooms out.
    ...
    // A cell that lands below one canvas pixel would divide by ~0 below. Zoom is only that small
    // before the first resize, when the canvas has no area and the counts do not matter.
