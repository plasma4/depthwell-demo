...
/// 1.0 is one world pixel, the same grid a block sprite's texels sit on,
/// so one block holds 16x16 background pixels and one chunk holds 256 per edge.
...
fn publishBackgroundGrid(effective_zoom: f64, canvas_w: f64, canvas_h: f64) void {
    ...
    // keeps a cell a whole number of world pixels and keeps the grid's phase: a coarser cell is the
    // exact 2x2 of a finer one, so the pattern does not slide sideways as the camera zooms out.
