    ...
    for (0..ch) |gy| {
        ...
        for (0..cw) |gx| {
            ...
            if (player_coord.moveAtDepth(.{ offset_x, offset_y }, pass.depth)) |target_coord| {
                ...
                // Read in place: a `Chunk` is 4 KiB, and copying one per visible chunk per frame
                // costs more than the block pass that follows it.
...
fn applyVariation(out: []memory.Block, wb: u32, frame: u32) void {
    // Walked row by row rather than by flat index: the tile coordinates are the only thing the index was
    // ever for, and recovering them per block costs a divide and a modulo on every cell of the screen.
