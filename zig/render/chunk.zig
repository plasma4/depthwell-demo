...
///
/// Same world-to-screen mapping `placePlayer()` uses, so a queued entity is pixel-aligned with
/// the tile under it: one viewport pixel is `CHUNK_SIZE` subpixels, scaled by the layer's zoom.
