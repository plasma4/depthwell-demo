...
const IndicatorKind = enum {
    ...
    ///
    /// Spectating is read-only, so every mutating (menu-backed) indicator goes dead. The two depth
    /// indicators stay live: `.invportal` keeps going further back, and ANY `.portal` acts as the way
    /// down. The clicked portal is not entered (that would reframe the depth); it only asks for the
    /// recorded route back, which is why every one of them serves equally. See `portal.triggerReturn()`.
