...
///
/// Called when a depth transition begins (see `portal.beginTransition()`):
/// a menu is bound to a block at the depth being LEFT,
/// so acting on one mid-animation would write a modification into a world that is about to be replaced.
/// `drawIndicators()` bails out for the whole transition, so nothing can reopen one until it lands.
