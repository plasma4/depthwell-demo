...
//!
//! One block window scan feeds both the per-frame drawing pass and the hover test; see `scanIndicators()`.
...
const IndicatorKind = enum {
    ...
    ///
    /// Every depth is fully playable, so each indicator is live at each depth, and `ref` is not read.
...
fn indicatorGeom(
    ...
    // Reading `game.player_pos` and `game.camera_scale` here instead made an indicator pop once
    // per tick while it glided, which is only visible once a tick is worth many frames.
