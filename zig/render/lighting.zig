...
test "chroma survives the trip, and CHROMA_WHITE_MIX is what pulls it down" {
    const got = roundTrip(.{ .hue = Hue.violet, .chroma = 1.0 }, 255);
    const recovered = @as(f32, @floatFromInt(got.c)) / @as(f32, @floatFromInt(LIGHT_MAX));
    try testing.expectApproxEqAbs(1.0 - CHROMA_WHITE_MIX, recovered, 0.05);
}
