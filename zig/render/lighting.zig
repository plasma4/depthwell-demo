...
// --- Player-only light, for the simulation rather than the screen ---
//
// Everything above this line runs per RENDER frame off interpolated positions, which is right for a
// picture and wrong for a rule: two players on the same logic tick would disagree about what they may
// mine depending on when their GPU got around to a frame. So the light mining gates on is flooded here
// instead, on the logic tick, out of the same cost model but seeded from the committed player position.
...
/// Indexed by `miningIndex()`; only meaningful for the tick stamped in `mining_light_frame`.
...
fn floodMiningLight() void {
    ...
    // The window is filled by walking chunks, but read back by block offset from the player, and a
    // mix-up between the two is invisible in the result (it stays a plausible-looking light field).
    // So in debug, check one deliberately asymmetric cell against the same block resolved the other way.
    if (dw.is_debug) {
    ...
    // The player's own position within their block, in window-cell units: the block they stand in sits
    // at MINING_RADIUS, and the subpixel remainder places them continuously inside it.
