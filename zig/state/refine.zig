...
//!    region's floor/ceiling row, and `protectsSurfaceCell()` stops the terrain parent on the far side
...
pub const Chain = struct {
    ...
    ///
    /// A chain covering 4x the blocks at every depth would be a curtain to the floor within a couple of
    /// descents, and a fixed length would make every vine in the world identical. A band does neither:
    /// the reach roughly doubles rather than quadrupling, and it is drawn per COLUMN, so a wide chain
    /// ends ragged. The `4x` ceiling is structural either way, since a child's run cannot exceed four
    /// times its parent's.
    min_length: u32,
    max_length: u32,
...
/// Whether a cell at region row `ly` may hold `sprite`, given the parent's neighbors.
///
/// The anchor check for a macro block that arrives by EVOLUTION rather than by a plan: `mossy_stone`
/// evolves into vine, and terrain fills its whole region, so without this a vein of it becomes a wall
/// (or, once the rock beside it erodes, a line) of vine hanging off nothing. Only the row that touches
/// the surface the sprite needs may take it, and only when that surface is actually there.
///
/// Sprites with no plan are terrain and answer true; the carve alone decides those.
...
/// carve may not take it. The promise is per CELL, not per row: a region holds at most a few copies, so
/// preserving all `BLOCKS_PER_PARENT` cells of a row would flatten ground under every bush in the world
/// for the sake of one or two cells. Both sides read the columns from `surfaceColumns()`, which is what
/// keeps "where the copy goes" and "where the ground stays" from drifting apart.
///
...
/// A chain does NOT come through here; what it needs above depends on its run, so `chainChild()` owns
/// that check.
...
/// `protectsSurfaceCell()` reads it for a NEIGHBORING parent, so the two can never disagree about which
/// cells a copy needs. A pure function of the parent's cell, its plan, and its tag.
fn surfaceColumns(rule: Rule, parent: Block, noise_seed: Vec2u, px: WorldCoord, py: WorldCoord) ColumnMask {
    switch (rule.plan) {
        ...
        .chain => |c| {
            // Hashed on the CEILING the chain hangs from (`run` cells up), the one cell every parent of
            // a chain agrees on, so the whole chain stays in the same columns all the way down.
            // Wrapping is correct and required: a coordinate is a position in a 2^69 space, and only its
            // consistency along the chain matters.
...
///
/// Keyed on the ceiling cell and the column, so every parent of the chain agrees on it (which keeps a
/// column continuous) while the columns beside it get their own answer (which is what makes a wide chain
/// end ragged instead of square).
inline fn columnReach(c: Chain, noise_seed: Vec2u, px: WorldCoord, ceiling_y: WorldCoord, lx: u4) u64 {
    ...
    const h = seeding.NoiseMix.lane(regionHash(noise_seed, px, ceiling_y, .reach), lx);
    return c.min_length + h % span;
}
...
fn chainChild(c: Chain, rule: Rule, ctx: Context) BlockSpec {
    ...
    // A chain HANGS, so what it needs above depends on where in the chain this parent sits: the top of
    // a chain needs real ceiling, and the rest of it needs more chain.
    //
    // Checking `solid OR more of the same sprite` instead (as `AnchorKind.suspended` does, since the
    // cascade sees one cell at a time) would let an unanchored run of vine keep itself alive: each cell
    // points at the one above it and nothing ever points at rock. That is exactly how vine inherited
    // from `mossy_stone` used to end up as a line hanging down the middle of an open cave.
...
test "a chain hangs from rock or not at all" {
    ...
    // The top of a chain sitting under MORE chain is not anchored to anything: this is vine inherited
    // from `mossy_stone`, and letting it through is what drew lines down the middle of open caves.
...
test "an evolution into a hanging block still needs a ceiling" {
    // `mossy_stone` evolves into vine. Terrain fills its whole region, so this is the check that keeps a
    // vein of it from becoming a wall of vine with nothing above it.
...
test "the terrain only pushes up under the cells a decoration really lands on" {
    ...
    // The protected cells must be exactly the columns the bush's own region claims: no more (which
    // would flatten the ground under every bush) and no fewer (which would drop the bush into a hole).
