...
/// and a rule that has to hold for every path (a vine needing a ceiling, a fresh chain starting its
/// run at 1) is stated here once rather than at each caller.
///
...
///
/// `chance` is honored exactly in expectation whether or not the cells clump: the blob field biases
/// the roll symmetrically about its own midpoint, and the bias is bounded by how much the odds can
/// absorb before reaching 0 or 1, so averaging the local odds over the world gives `chance` back.
...
test "an evolution's odds hold over the world, and only its arrangement changes" {
    ...
    inline for (.{
        ...
        // Neighbor pairs, which is what separates a patch from static: for independent rolls the
        // chance a converted cell's neighbor also converted is just the conversion rate itself.
        var pairs: usize = 0;
