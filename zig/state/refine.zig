...
pub fn evolve(source: Sprite, ctx: Context) Evolved {
    if (source == .mossy_stone and rollsMossDecay(ctx)) return .{ .id = .stone };
...
/// Fraction of ordinary moss that returns to plain stone at each depth.
const MOSS_DECAY_CHANCE = 0.16;
...
/// as it descends.
fn rollsMossDecay(ctx: Context) bool {
    const roll = FastHash.float2d_32(
        ctx.noise_seed,
        seeding.foldWorld(ctx.wx +% @intFromEnum(Salt.moss_decay)),
        seeding.foldWorld(ctx.wy -% @intFromEnum(Salt.moss_decay)),
    );
...
test "an evolution's odds hold over the world, and only its arrangement changes" {
    ...
    inline for (.{
        ...
        .{ Sprite.mossy_stone, 0.664 }, // spread plus the independent return-to-stone roll
    }) |case| {
    ...
    for (0..100) |py| {
        for (0..BLOCKS_PER_PARENT) |lx| {
            ...
            if (top.id != .spiralvine) {
                try testing.expectEqual(Sprite.more_mossy_stone, lower.id);
                none += 1;
