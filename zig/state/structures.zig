...
///
/// Laid out one full `SimBuffer` sweep row wide (rather than square) for the same slot count:
/// a bulk pass walks chunk rows left to right, so a row that fits keeps every neighbor it is about to
/// ask for, and the four rows of height cover the 3 the halo can reach.
