...
//! - `spawn_area` (power of two), `max_w`, `max_h`, `target_chance` — the spawn grid and density.
//! - `getBounds()` — where in its grid cell the candidate lands.
//! - `generate()` — the sprite for one block of the footprint.
//! - `fits()` (OPTIONAL) — terrain constraints for the candidate as a whole (must it rest on ground? stay
//!   clear of a ceiling?). Evaluated ONCE per grid cell and memoized, so constraints may sample terrain
//!   freely without the cost scaling by footprint area. Build them from `baseSolid()`/`surfaceY()`.
...
/// The shared primitive every terrain constraint is built from; see `fits()` in the structure interface.
...
/// Finds the ground surface of column `wx`: the topmost row within `span` of `probe_y` that is solid base
/// terrain with open space directly above it. Null when the column has no surface in that window
/// (solid all the way through, or empty all the way through), which reads as "no floor here".
/// Costs `2 * span + 2` terrain samples, so keep `span` small and gate it behind cheaper checks.
pub fn surfaceY(wx: i32, probe_y: i32, span: i32) ?i32 {
    std.debug.assert(span > 0);
    var above_solid = baseSolid(wx, probe_y - span - 1);
    var y = probe_y - span;
    while (y <= probe_y + span) : (y += 1) {
...
/// or rejected by its own terrain constraints. Keyed by (`cx`, `cy`, `seed`);
...
inline fn generateStructureForKind(
    ...
    if (entry.bounds) |bounds| {
        ...
        if (i_wx >= bounds.x_start and i_wx < bounds.x_end and i_wy >= bounds.y_start and i_wy < bounds.y_end) {
            // Terrain constraints are a property of the CANDIDATE, not of the cell, so evaluate them once
            // per grid cell and cache the verdict: the whole footprint reuses it, and a rejected candidate
            // never samples terrain again. (A per-cell `fits()` would re-run the noise for every body block.)
            if (comptime @hasDecl(S, "fits")) {
                const fits = entry.fits orelse blk: {
                    const v = S.fits(bounds);
                    entry.fits = v;
                    break :blk v;
                };
                if (!fits) return null;
