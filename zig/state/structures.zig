...
///
/// Off by default, and it must stay off for anything shaped: two overlapping trees interleave into
/// one unreadable blob. It exists for a REGION-like kind (see `structures/Deposit.zig`), where every
/// placement in a neighborhood draws the same material, so two overlapping blobs are
/// indistinguishable from one larger one. At the density such a kind runs at, the same-kind rule was
/// deleting roughly half of it to prevent something invisible.
///
