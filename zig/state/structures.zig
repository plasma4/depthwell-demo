...
/// a seating scan reaches rows below it, and `isBlockedBySameKind()` resolves the candidate in (`cx - 1`, `cy - 1`)
///
...
pub const adjusted_chances = blk: {
    ...
    for (Configs, 0..) |conf, i| {
        ...
        // (see `placedBounds()`), and terrain acceptance is unknowable at compile-time.
        // So lambda badly overestimates for any gated structure, and an uncapped compensation would drive every dense structure's chance to 1.0 -- silently turning `target_chance` into a dead knob. Cap the boost.
