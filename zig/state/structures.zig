...
/// Runs every rule in `list` against `bounds`. Shared with `decorations.zig`.
pub inline fn satisfies(comptime list: []const Constraint, bounds: Rect) bool {
    inline for (list) |c| {
...
inline fn computeStructureBounds(comptime kind: usize, cx: i32, cy: i32, struct_seed: Vec2u) ?Rect {
    ...
    inline for (0..Configs[kind].attempts) |_| {
        if (S.getBounds(&state, cx, cy)) |bounds| {
            if (satisfies(constraint_table[kind], bounds)) return bounds;
        }
    }
    return null;
}
