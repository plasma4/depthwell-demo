...
/// Both terms are first-order approximations, so treat `target_chance` as a dial and measure the result.
pub const adjusted_chances = blk: {
    ...
    for (Configs, 0..) |conf, i| {
        ...
        for (0..i) |j| {
            ...
            const p_blocked_by_prev = density_prev * shadow_w * shadow_h;
        ...
        // which is (to first order) half of its own overlap shadow.
        ...
        p_not_blocked *= @max(0.0, 1.0 - density_self * self_shadow);

        if (p_not_blocked <= 0.0) {
            @compileError("Structure blocking probability is 100%; structure cannot spawn.");
        }
        raw[i] = @min(1.0, conf.target_chance / p_not_blocked);
    }
    break :blk raw;
};
...
fn isBlockedBySameKind(comptime kind: usize, cx: i32, cy: i32, bounds: Rect, struct_seed: Vec2u) bool {
    ...
    while (ncy <= cy + 1) : (ncy += 1) {
        ...
        while (ncx <= cx + 1) : (ncx += 1) {
            ...
            if (!earlier) continue;
            if (getStructureBounds(kind, ncx, ncy, struct_seed)) |other| {
