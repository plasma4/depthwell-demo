...
/// A cell is `spawn_area` blocks wide, so even the smallest kind's tile spans far more world than
/// one generation pass touches.
...
/// Tiled, so a chunk and its 8 neighbors land in 9 distinct slots ALWAYS. That is the property this
/// cache exists for: the edge-flag halo walks a chunk's border cell by cell, alternating between the
/// left and right neighbor on every row, and a hashed index collides somewhere in a 3x3 about half
/// the time at 64 slots, which is exactly the thrash it was sized to avoid.
