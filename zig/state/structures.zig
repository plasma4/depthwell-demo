...
pub const Edge = struct {
    ...
    inline fn resolve(self: Edge, start: i32, end: i32) i32 {
...
test "TEMP final invariants" {
    dw.memory.game.seed2 = @splat(0);
    for (0..32) |i| dw.memory.game.seed2[i] = 0x9E3779B97F4A7C15 *% (i + 7);
    ...
    inline for (0..structures.len) |kind| {
        ...
        while (cy < 320) : (cy += 1) {
            var cx: i32 = 200;
            while (cx < 320) : (cx += 1) {
                const b = placedBounds(kind, cx, cy, seed) orelse continue;
                if (isBlockedBySameKind(kind, cx, cy, b, seed)) continue;
                if (kind > 0 and isBlockedByHigherPriority(kind, b, seed)) continue;
                try rects.append(alloc, b);
            }
        }
        for (rects.items, 0..) |a, i| {
            for (rects.items[i + 1 ..]) |b| try std.testing.expect(!a.overlaps(b));
        }

        if (kind == getStructureIndex(Chamber)) {
            for (rects.items) |b| {
                // every block of the footprint comes back correct through the real entry point
                const room_h = 6;
                var y = b.y_start;
                while (y < b.y_end) : (y += 1) {
                    var x = b.x_start;
                    while (x < b.x_end) : (x += 1) {
                        const got = addStructures(.none, @bitCast(x), @bitCast(y), seed).id;
                        const ry = y - b.y_start;
                        const want: Sprite = if (ry >= room_h) .black_plate else if (x == b.x_start or x == b.x_end - 1 or ry == 0 or ry == room_h - 1)
                            .black_plate
                        else if (ry == room_h - 2) .lava_stone else .none;
                        if (want != got) {
                            std.debug.print("\nBLOCK MISMATCH at ({d},{d}) rel({d},{d}): want {s} got {s}\n", .{ x, y, x - b.x_start, ry, @tagName(want), @tagName(got) });
                            return error.BlockMismatch;
                        }
                    }
                }
                // SEATED: no column's ground rises above the room's base row (never buried),
                // and every column has solid ground within the plinth's reach (never floating).
                const room_base = b.y_end - 4;
                var x = b.x_start;
                while (x < b.x_end) : (x += 1) {
                    if (baseSolid(x, room_base - 1)) {
                        std.debug.print("\nBURIED: col {d} solid at room_base-1 ({d})\n", .{ x, room_base - 1 });
                        return error.Buried;
                    }
                    if (surfaceY(x, room_base, room_base + 4) == null) {
                        std.debug.print("\nFLOATING: col {d} has no ground in plinth reach\n", .{x});
                        return error.Floating;
                    }
                }
            }
        }
        if (kind == getStructureIndex(Shrub)) {
            for (rects.items) |b| {
                const l = addStructures(.none, @bitCast(b.x_start), @bitCast(b.y_start), seed).id;
                const r = addStructures(.none, @bitCast(b.x_start + 1), @bitCast(b.y_start), seed).id;
                try std.testing.expectEqual(l.supports()[1].sprite, r);
            }
        }
        std.debug.print("{s:<9}={d} ", .{ @typeName(structures[kind])[19..], rects.items.len });
    }
    std.debug.print("ALL SEATED + PAIRED + NO OVERLAPS\n", .{});
}
