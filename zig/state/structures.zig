...
//! - `getBounds()`: where in its grid cell the placement lands, or null to reject the cell outright; use `jitter()` so the box is free to overhang the cell
...
//!
//! `target_chance` is a ROLL, not a density: terrain rules throw most rolls away, and how many is unknowable
//! at compile-time. There is no model correcting for that -- there was one, and it could not see the term that
//! dominates, so it only ever nudged by 10% or slammed into its own cap. Set the roll, then run
//! `debug/audit.zig` and read the density off. That is the loop.
//!
...
// A constraint list is evaluated against a comptime `probe` -- a `fn (i32, i32) bool` testing one block.
// Structures probe RAW terrain (`baseSolid()`); decorations probe the foundation left AFTER structures,
// since they sit on top of whatever a structure built.
...
///
/// Cell order (`(cy, cx)` ascending) would be simpler but it is DIRECTIONAL: a box near its cell's left edge
/// overlaps the left neighbor more often, so it loses more often, and survivors drift right. Measuring it
/// showed the effect is small in practice, but a spatial tie-break has no business having a direction at all,
/// and a hash costs one draw.
...
///
/// Non-recursive on purpose: a rival is judged on whether IT would stand, never on whether it is itself
/// beaten. Chaining that would be unbounded across cells. The cost is mild over-suppression when the rival
...
///
/// Never holds a cache POINTER across the scan: `isBeaten()` re-enters `structCacheSlot()` for neighboring
/// cells of this same kind, which share this bank and can evict this very slot.
...
///
/// Keyed on the CANDIDATE's cell, never the block's own: a structure may overhang into the next cell, and
/// keying on the block would hand its far half a different roll (a `moss_shrub2` left with a `moss_shrub1_right` right).
///
/// The stream is INDEPENDENT of the one `computeStructureBounds()` drew from (a separate `unique_id` namespace).
/// It has to be: with `attempts`, that stream burned an unknowable number of draws before landing on the
/// winning box, so `generate()` could never replay to the right position. It also stops a structure's own
/// choices from correlating with its jitter offset.
