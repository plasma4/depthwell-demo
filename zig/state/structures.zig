...
//! - `getBounds()`: where in its grid cell the candidate lands, or null to reject the cell outright; use `jitter()` so the box is free to overhang the cell
//! - `generate()`: the sprite for one block of the footprint, derived from the passed `bounds`
//! - `constraints` (optional): spawning constraints evaluated ONCE per grid cell and memoized (and comptime-sorted cheapest first)
//!
//! A rule a structure can check WHILE computing its bounds belongs in `getBounds()`, not in `constraints`:
//! constraints only run once bounds exist, so by then any terrain scan `getBounds()` did is already paid for.
//! Rejecting there lets an expensive scan bail early (see `Chamber.getBounds()`).
//!
//! A candidate is anchored uniformly ANYWHERE in its `spawn_area` cell and may overhang into the neighboring cells.
...
pub const structures = .{
    ...
    @import("structures/Shrub.zig"),
    @import("structures/Plant.zig"),
};
...
/// `base` being `.none` means "fall back to the natural terrain it replaced".
pub const StructureResult = struct {
    id: Sprite,
    base: Sprite = .none,
    /// Starting water volume (0-15) for a waterloggable `id` placed inside a pool.
    /// A structure that puts a dry waterloggable block (a chest, a furnace) in a row it also fills with water MUST set this to `Block.MAX_HP`,
...
///
/// Probing OUTSIDE the world is routine, not exceptional: a constraint reaches a row above its box, a
/// seating scan reaches rows below it, and `isBlockedBySameKind()` resolves the candidate in cell
/// (`cx - 1`, `cy - 1`) -- all of which fall off the world near its corner. There is no ground out there, so
/// this reports the void as non-solid, which fails every `.solid` rule and keeps structures off the border.
///
/// The bounds check is also load-bearing, not just semantic: world coordinates are `u32`, so a negative
/// `wx`/`wy` reinterprets to roughly 4 billion, and the noise multiplies coordinates (`y * 2` in
/// `getFbmValue()`) without wrapping -- an out-of-world probe used to trap on integer overflow.
...
/// Two shadows eat into a structure's target density: higher-priority kinds (which suppress it outright) and its OWN kind
/// (overhang means two adjacent candidates can collide, and the later cell loses).
/// Terrain `constraints` are NOT modelled here (they are data-dependent), so a gated structure always lands under its `target_chance`:
/// treat the dial as an upper bound and measure the result.
...
/// One memoized structure grid cell: its bounds and (lazily) whether the candidate is suppressed by a colliding structure or rejected by its own terrain constraints.
/// Keyed by (`cx`, `cy`, `seed`); `blocked`/`fits` stay null until first computed for that cell.
...
/// candidate, so blocking on mere existence would let a heavily terrain-gated kind -- one that almost never
/// actually places -- blanket-suppress every lower-priority kind in the world.)
///
...
/// The candidate in cell (`cx`, `cy`) if it exists AND survives collisions AND fits the terrain: everything
/// about a candidate that does NOT depend on which block is asking. Memoized per cell in `struct_cache`.
///
/// Never holds a cache POINTER across a scan: `isBlockedBySameKind()` re-enters `structCacheSlot()` for
/// neighboring cells of this same kind, which share this bank and can evict this very slot -- a held pointer
/// would then be reading and writing some other cell's entry. So copy out, scan, re-acquire.
...
/// Draws one block of an accepted candidate.
///
/// The hash is replayed from the CANDIDATE's cell, never from the block's own: a structure may overhang into
/// the next cell, and keying on the block would hand its far half a different roll (a `moss_shrub2` left with
/// a `moss_shrub1_right` right). This matches the stream `computeStructureBounds()` drew `bounds` from.
inline fn generateFrom(
    ...
    var state = makeStructureHash(
        struct_seed,
        @bitCast(candidate.cx * i_area),
        @bitCast(candidate.cy * i_area),
        S.spawn_area,
        kind,
    );
    _ = state.getChance(adjusted_chances[kind]);
    return S.generate(starting_sprite, wx, wy, candidate.cx, candidate.cy, candidate.bounds, &state, struct_seed);
}
...
///
/// `addStructures()` is asked the same question 256 times per chunk (once per block), but the answer -- which
/// candidates exist, survive collisions, and fit the terrain -- is a property of the CHUNK, not the block.
/// Answering per block meant re-hashing into `struct_cache` thousands of times per chunk to re-derive an
/// identical verdict; answering per chunk reduces the per-block cost to a handful of rect containment tests.
...
/// Direct-mapped context cache (slots must be a power of two). Sized so a chunk AND its neighbors stay
/// resident: the edge-flag halo walks a chunk's border cell by cell, alternating between the left and right
/// neighbor chunk on every row, so a single-entry memo would thrash.
const CHUNK_CTX_SLOTS = 16;
...
fn buildChunkCandidates(ctx: *ChunkCandidates, chunk_x: i32, chunk_y: i32, struct_seed: Vec2u) void {
    ...
    inline for (0..structures.len) |kind| {
        ...
        while (cy <= ys.hi) : (cy += 1) {
            var cx = xs.lo;
            while (cx <= xs.hi) : (cx += 1) {
                // Reject a candidate that misses this chunk BEFORE paying for its collision and terrain
                // verdicts, exactly as the per-block scan did with its `contains()` check.
...
pub fn addStructures(
    ...
    inline for (0..structures.len) |kind| {
        for (ctx.list[kind][0..ctx.counts[kind]]) |candidate| {
            ...
            if (generateFrom(kind, candidate, starting_sprite, wx, wy, struct_seed)) |result| {
                return result;
            }
        }
    }
...
/// Reference implementation of `addStructures()`: resolves the covering candidate from scratch for every
/// single block, the way the generator did before `ChunkCandidates` hoisted that work to the chunk.
/// Exists only to hold the prepass honest (see the test below).
