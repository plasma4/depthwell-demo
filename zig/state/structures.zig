...
//!
//!   1. ROLL     `target_chance` decides whether the cell even attempts a placement.
//!   2. ANCHOR   `getBounds()` picks the box, or the default jitter does it when the size is fixed.
//!   3. SEAT     `seat` slides the box down onto the terrain surface (optional).
//!   4. GATE     `constraints` accept or reject the finished box (optional).
//!
...
//! - `getBounds()`: only when the box is not a plain `max_w`-by-`max_h` rectangle (a rolled size, say).
...
//! because constraints are pure predicates over a box that is already final. Standing on the ground is
//! the common case of that, which is what `Seat` exists for. `getBounds()` remains the escape hatch for
//! anything else, and rejecting there lets an expensive scan bail before the box is even complete.
//!
...
///
/// This cannot be a `Constraint`, and the distinction is the one worth internalizing:
/// a constraint is a pure PREDICATE over a box that is already final, while seating is a TRANSFORM that
/// computes where the box belongs. `checkConstraint()` returns a bool and has no way to move anything.
///
/// Seating also makes the "is there ground under my corners" constraints redundant:
/// a seated box has, by construction, proven that every profiled column has a surface at the seated row.
/// Do not write both.
///
...
///
/// The reference row is the one directly BELOW the box (`y_end` is exclusive), which is what makes a
/// structure stand ON the ground instead of sinking into it: seating so the box's own last row coincided
/// with the surface would bury that row a block deep.
///
/// Costs `(x1 - x0) * (max_drop + 2)` terrain samples in the worst case, but a candidate over unsuitable
/// ground normally dies on its first or second column.
...
pub const Configs = blk: {
    ...
    for (structures, 0..) |S, i| {
        // Vertical REACH, not footprint height: a seated structure may sit up to `max_drop` rows below its
        // anchor, and every scan that asks "which candidate could cover this block?" has to see that far.
        // Folding it in here is what lets a structure declare only how tall its art is; getting this wrong
        // by hand used to cost a structure its bottom rows near a chunk border.
...
inline fn computeStructureBounds(comptime kind: usize, cx: i32, cy: i32, struct_seed: Vec2u) ?Rect {
    ...
    inline for (0..Configs[kind].attempts) |_| {
        ...
        if (anchored) |raw| {
            // SEAT, then GATE. Seating runs first because a constraint judges the FINAL box, and an
            // unseated box is not final: gating before the slide would test the wrong rows entirely.
