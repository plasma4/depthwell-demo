...
/// Requires a structure to be walled in by solid terrain, whatever its shape.
///
/// The problem this solves: "surrounded by rock" is easy for a rectangle and awkward for anything else.
/// Checking a bounding box around a circle tests the wrong blocks (the corners are far outside it),
/// and hand-writing a halo per shape does not scale as structures are added.
///
/// So the structure hands over its own shape instead, and the halo is derived from it:
/// - walk the region (by default the footprint grown by one block on every side)
/// - a block the shape does NOT occupy, but which touches one it DOES, is a halo block
/// - every halo block must be solid base terrain
///
/// `covers()` is the same shape predicate a structure's `generate()` already implements,
/// so declaring it is usually just factoring out a test that exists anyway (see `Geode.zig`).
pub const Encase = struct {
    /// Shape test in ABSOLUTE world blocks, so it can be called for halo blocks outside the footprint.
    /// Must agree with `generate()` about which blocks the structure occupies, or the halo covers the wrong ring.
    covers: *const fn (wx: i32, wy: i32, bounds: Rect) bool,
    /// How many halo blocks may be open (non-solid) before the candidate is rejected.
    /// 0 demands a perfect seal. A small budget allows PARTIAL encasement, where the structure may poke out
    /// of a cave wall a little, which often reads better than a hermetic seal (see the geode's exposed rim).
    /// The scan bails the moment the budget is blown, so a generous budget is also a cheaper budget.
    max_open: u32 = 0,
...
pub const Constraint = union(enum) {
    ...
    level: Level,
    /// The structure must be walled in by solid terrain on every side of its own shape; see `Encase`.
    encase: Encase,
        ...
        .encase => |en| {
            ...
            while (y < y1) : (y += 1) {
                ...
                while (x < x1) : (x += 1) {
                    ...
                    if (!probe(x, y)) {
                        open += 1;
                        if (open > en.max_open) return false;
                    }
                }
            }
            return true;
...
fn constraintCost(comptime c: Constraint, comptime w: i32, comptime h: i32) usize {
    ...
    return switch (c) {
        ...
        // Two shape calls per region block, plus the terrain sample: dearer than a plain span scan.
        .encase => |en| @intCast(spanOf(en.region.x0, en.region.x1, w) * spanOf(en.region.y0, en.region.y1, h) * 3),
