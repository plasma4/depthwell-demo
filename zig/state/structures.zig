...
pub fn satisfies(comptime probe: anytype, comptime list: []const Constraint, bounds: Rect) bool {
    inline for (list) |c| {
...
///
/// Two cells that share a slot are told apart by the full (`cx`, `cy`) stored in the entry, so a collision
/// costs a recompute and never a wrong answer. The deep world's `u64` chunk suffix cannot reach that
/// comparison either way: structures are generated ONLY at `STARTING_ZOOM_TIMES` (see
/// `world.computeBaseFoundation()`), where a block coordinate is bounded by `MAX_WORLD_BLOCK` and every
/// deeper depth inherits its material through `ancestor.zig` instead of placing structures of its own.
...
pub fn acceptedBoundsForAudit(comptime kind: usize, cx: i32, cy: i32, struct_seed: Vec2u) ?Rect {
