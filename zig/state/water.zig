...
var cells_changed: std.StaticBitSet(world.SIM_GRID_SIZE_SQ) = undefined;
...
/// `grid_idx` is the cell's index in the `SIM_GRID_SIZE` x `SIM_GRID_SIZE` sim grid (`ry * SIM_GRID_SIZE + rx`).
///
...
pub fn tickWater() void {
                    ...
                    if (down_ptr) |dp| {
                        if (dp.isFlowable()) {
                            ...
                            if (dest_vol < MAX_HP) {
                                ...
                                setVolumeAt(dp, dest_vol + amt, idx + world.SIM_GRID_SIZE);
    ...
    while (dirty_it.next()) |idx| {
        ...
        // Persist only the cells this tick actually moved. A settling pool re-levels over many ticks and
        // touches a handful of cells each one, so writing the whole chunk back would cost 4 KiB per tick
        // AND make every cell of the chunk authored (freezing its terrain against future generator changes).
        const writer = world.mod_store.beginWrite(key);
        const cached = world.chunk_cache.findIndex(coord);
        for (0..CHUNK_SIZE) |by| {
            const row_base = (@as(usize, dy) * CHUNK_SIZE + by) * world.SIM_GRID_SIZE + @as(usize, dx) * CHUNK_SIZE;
            for (0..CHUNK_SIZE) |bx| {
