...
///
/// `dx` and `dy` are `comptime` so `getFlagBit()` folds to a single constant,
/// but the function is deliberately NOT `inline`:
/// the 3x3 unroll in `applyCellWaterFlags()` would otherwise paste eight copies of the
/// packed `Block` accessors into one function, and LLVM cost is quadratic in basic-block count.
fn neighborEdgeBit(
    comptime dx: i32,
    comptime dy: i32,
    curr: *Chunk,
