...
/// Direction the row sweep of phase 2 runs this tick. It flips every tick.
///
...
/// The file treats an absent neighbor as present, so a cell at the `SimBuffer` border
/// never sees open space outside the buffer and drains into it.
