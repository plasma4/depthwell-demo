...
/// Consecutive ticks each slot has been dirty, for `TRACE_UNSETTLED`.
/// A pool that is genuinely flowing drains or fills in a bounded number of ticks; one that keeps
/// reporting movement forever is oscillating, which is what a chunk visibly flickering between two
/// water states looks like from the render side.
var unsettled_ticks: [SIM_BUFFER_SIZE]u16 = @splat(0);

/// Ticks a slot may stay continuously dirty before it is reported. Generous enough that a real pool
/// filling across the whole buffer never trips it.
const UNSETTLED_LIMIT: u16 = 600;

/// Logs chunks that never settle. Debug builds only, and off by default: this is a diagnostic for the
/// "a chunk flickers between two water states and then abruptly stops" case, where the sim is trading
/// the same units back and forth instead of converging.
const TRACE_UNSETTLED = false;

/// Resets all water simulation states, clearing pending flag updates and tracking bitsets.
pub fn reset() void {
    unsettled_ticks = @splat(0);
...
pub fn tickWater() void {
    ...
    while (act_it.next()) |idx| {
        if (!dirty_chunks.isSet(idx)) {
            ...
            const settled_idx = SimBuffer.getIndex(ax, ay);
            if (dw.is_debug and TRACE_UNSETTLED) unsettled_ticks[settled_idx] = 0;
            SimBuffer.water_settled.set(settled_idx);
        }
    }
    ...
    while (dirty_it.next()) |idx| {
        ...
        if (dw.is_debug and TRACE_UNSETTLED) {
            unsettled_ticks[sim_idx] +|= 1;
            if (unsettled_ticks[sim_idx] == UNSETTLED_LIMIT) {
                dw.logger.warn(@src(), "Water chunk ({d},{d}) has not settled in {d} ticks; likely oscillating.", .{
                    coord.suffix[0],
                    coord.suffix[1],
                    UNSETTLED_LIMIT,
                });
