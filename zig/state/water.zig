...
//! a falling stream splitting into alternating occupied/empty (or tiny-volume) vertical cells.
//! Mid-stream cells therefore pour at full rate (see the gravity phase of `tickWater()`),
//! so a stream that merges with residual droplets heals instead of preserving the gaps,
//! and airborne cells never spread laterally (they hold until resting on support),
//! so a stream that momentarily backs up cannot spray sideways into alternating shelves.
//! A cell-per-tick automaton can only fully avoid parity for sources that emit every tick,
//! so intermittent sources (placement cadence gaps, 1-unit lateral cascades spilling over an edge)
//! may still produce it occasionally.
