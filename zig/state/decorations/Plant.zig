...
pub fn generate(local_x: i32, local_y: i32, state: *HashState) ?Sprite {
    _ = local_x;
    _ = state;
    // Top cell is the flowering tip; every cell below is shaft. The `cornflower`/`plant_base` support rules
    // in `types/sprite.zig` pin this stack together (see `Sprite.supports()`).
