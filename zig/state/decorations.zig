...
fn anchored(comptime kind: usize, ax: i32, ay: i32, seed: Vec2u) bool {
    ...
    // A multi-cell kind must own its WHOLE footprint or it renders as a fragment (a `moss_shrub1` with no
    // `moss_shrub1_right`). `resolve()` hands any shared cell to the higher-priority kind, so decline if one
    // claims a cell here. A 1x1 kind cannot fragment, so it skips the scan (priority order already hides it).
...
///
/// The decoration analogue of `structures.isBeaten()`: an outranked candidate steps aside rather than leave a
/// half-drawn fragment. Bounded, not unbounded-recursive: it consults only strictly-higher kinds, and
/// `anchored()` on those recurses into still-higher kinds only, so the kind index strictly decreases to 0.
