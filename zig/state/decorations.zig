//! All decoration placement, in one priority order. The cheap tier: small things that need no collision grid.
//!
...
//!
//! That works because a roll is keyed on the anchor's world position rather than drawn from a per-chunk RNG
//! stream. A position-keyed roll is ADDRESSABLE: a block can ask about an anchor in the chunk next door
//! without replaying that chunk. Cross-chunk coherence comes out free, and each kind owns an independent
//! probability instead of carving a slice out of one shared roll (which silently re-weighted its neighbors
//! every time a kind was added).
//!
//! Two shapes live here, because they are genuinely different algorithms:
//! - a POINT decoration has a fixed `size_x` x `size_y` footprint growing right and down from its anchor
//! - a COLUMN feature is a variable-length chain hanging off a surface (`Vine`)
//!
//! A column CANNOT be a point: resolving one per block means scanning up to `max_length` rows to find its
//! anchor and re-deriving every grow roll below it, which is quadratic in the chain length. Walking the column
//! once and carrying the state down it is O(1) per cell. Columns are stamped first, so a chain is never
//! punched full of holes by ground clutter.
//!
...
/// A 1x1 decoration standing on the ground: the common case, so it is a one-liner rather than a file.
...
///
/// NOT `structures.baseSolid()`, which sees only raw terrain because a structure must never ask what another
/// structure placed. Decorations run after structures, so they can ask -- and must, or a plant would happily
/// root itself in mid-air inside a room a structure carved out of solid rock.
...
/// would stand, never on whether it is itself beaten. Free for a 1x1 kind, which has no shadow to check.
...
///
/// Walks every anchor whose footprint could reach this block, in kind priority order, so a tall plant claims
/// its own base before a bush can. Costs one hash per candidate anchor in the common case, since the roll
/// fails long before any terrain is touched.
...
///
/// Columns first, then points, and neither ever overwrites an occupied cell. Reads no edge flags: a
/// decoration probes terrain itself, so it neither depends on nor perturbs them, which is what keeps a
/// neighbor's edits from shuffling this chunk's decorations.
...
/// A variable-length chain growing off a surface: it anchors with `anchor_odds`, then each further cell
/// continues with `grow_odds` until it misses or hits `max_length`.
...
test "a multi-block decoration owns its whole footprint" {
    ...
    // The bug this guards: `addDecorations()` and the tier were two passes with no shared order, so a bush
    // could take the ground cell out from under a plant and leave its stem floating. One priority order now
    // resolves every cell, so a standing anchor must own every block of its footprint.
    var checked: usize = 0;
    inline for (points, 0..) |D, kind| {
        if (D.size_x > 1 or D.size_y > 1) {
            var ay: i32 = 0;
