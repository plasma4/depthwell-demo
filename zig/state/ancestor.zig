...
/// Returns a donor for an empty parent corner that foundation terrain encloses.
/// Structure and decoration neighbors cannot donate an infill block.
fn enclosedInfillSource(n: [8]Block, lx: u4, ly: u4) ?Block {
    ...
    for (n) |block| if (!block.isFoundation()) return null;
...
///
/// Process:
/// 1. Fully enclosed empty parents can fill child corners from foundation terrain.
/// 2. Edge stone, liquids, and non-foundation blocks use special rules.
...
///
/// An empty parent can fill only through `enclosedInfillSource()`.
pub fn applyAncestorLogic(
    ...
    // Step 1. Fill only corners of a fully enclosed foundation hole.
...
test "infill: only enclosed foundation terrain can donate" {
    ...
    try testing.expect(enclosedInfillSource(portal_neighbor, 0, 0) == null);
}
