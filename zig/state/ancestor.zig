...
///
/// The product needs 69 bits (`seeding.WORLD_COORD_BITS`), which is why noise takes a `WorldCoord`
/// and not a `u64`: see `seeding.foldWorld()` for how it reaches a 64-bit hash without a period.
...
fn warpField(noise_seed: dw.utils.Vec2u, wx: WorldCoord, wy: WorldCoord) dw.utils.Vec2f32 {
    ...
    const coarse = procedural.getDualValueNoise(
        noise_seed,
        wx,
        wy,
        1.0 / MATERIAL_WARP_SCALE,
    );
    const fine = procedural.getDualValueNoise(
        noise_seed,
        wx,
        wy,
