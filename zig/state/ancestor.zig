...
inline fn inheritedLiquidVolume(parent_volume: u4, ly: u4) u4 {
    // A settled pool rests at `water.RESTING_VOLUME`, not at `MAX_HP`, so a parent there is FULL water
    // rather than a surface that has room left. Splitting it would empty the region's top row, and that
    // row is inherited as a partial parent next depth, so an ocean would shed a row per depth increase
    // wherever the sim is not running to fill it back in (anywhere outside the `SimBuffer`).
...
pub fn applyAncestorLogic(
    ...
    if (parent_sprite.isLiquid()) {
        // Only a SURFACE parent carries a waterline worth splitting. A parent with liquid above it is
        // interior water: whatever its volume happens to be is just where the sim left it, and splitting
        // that reads a settled ocean as a stack of part-empty surfaces, carving an air row into the
        // middle of it. That row is then inherited as its own partial surface, so the body loses another
        // row every depth increase, and the sim can only paper over it where the `SimBuffer` reaches.
        // Neighbors are row-major with the center removed, so index 1 is the parent directly above.
        ...
        // A real surface: `hp` IS the liquid's volume, so refining one cell into a region SPLITS that
        // volume rather than handing every child a full one; see `inheritedLiquidVolume()`.
...
pub fn getInheritedMaterial(key: DepthCoordinate, bx: u4, by: u4) Block {
    ...
    // The CURRENT depth belongs to the `SimBuffer` and the chunk cache, which the water simulation and
    // every block edit keep up to date. The `AncestorCache` is only ever dropped on a depth change
    // (`world.clearCaches()`), so a tier-0 entry is a materialization frozen at the moment it was
    // inserted: serving the current depth from it hands back terrain from before the last edit.
    //
    // That is what makes this an authoritative source rather than a cache detail. `addEdgeFlagsFractal()`
    // resolves its halo through `getCachedChunk()` and falls back HERE on a miss, so if the two disagree
    // the water flags it derives — and therefore what the shader draws — depend on which cache happened
    // to hold a neighbor. Routing the current depth to the same authority makes the two branches agree.
    ...
    if (target_depth == STARTING_ZOOM_TIMES) {
        ...
        // Skipped at the current depth, where an entry can outlive the edit that invalidated it.
        if (target_depth != memory.game.depth) {
    ...
    // Skipped at the current depth for the staleness reason above; deriving fresh is the only way to
    // stay in step with `mod_store` there.
    if (target_depth != memory.game.depth) {
        if (ancestor_cache.get(key)) |cached| return cached.blocks[block_idx];
    }
