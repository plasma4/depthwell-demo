...
pub fn applyAncestorLogic(
    ...
    //
    // Every other refined decoration makes the same demand of one ROW rather than the whole region
    // (`refine.protectsSurfaceRow()`): its own children rest against that row unconditionally, so it
    // has to be there, while the rest of this parent still erodes normally.
    if (!anchorsPortal(parent_neighbors) and
        !dw.refine.protectsSurfaceRow(parent_neighbors, ly) and
    ...
    return .{ .id = evolved_sprite, .base_id = base_id, .seed = noise_hash_2, .tag = tag };
}
