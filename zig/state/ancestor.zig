...
///
/// A modified chunk does NOT force a full materialization here: the lineage trace stays procedural and the
/// player's edit is overlaid on the single cell that was asked for, which keeps this O(1) on the hot
/// generation path. Cached chunks are already materialized, so they need no overlay.
pub fn getInheritedMaterial(key: DepthCoordinate, bx: u4, by: u4) Block {
    ...
    // A cache hit is already materialized (mods overlaid). A miss recurses procedurally and overlays this
    // cell's authored value below (see the `getCell()` call before returning), so no full-chunk mod lookup here.
    if (ancestor_cache.get(key)) |cached| return cached.blocks[block_idx];
