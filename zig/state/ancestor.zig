...
/// One axis of a block's absolute position at its own depth.
///
/// The product needs 69 bits and a `WorldCoord` holds 64, so the top bits wrap; the quadrant, which
/// would be a 65th bit of the chunk index, is dropped rather than wrapped in. Neither loses anything
/// a caller can observe: `noise_seed` in `applyAncestorLogic()` is already derived per quadrant and
/// per depth, so those never share a field regardless of the coordinate, and the wrap itself only
/// repeats at a distance no player crosses (see `seeding.WorldCoord`).
inline fn worldBlock(chunk: u64, block: u4) WorldCoord {
    return chunk *% dw.CHUNK_SIZE +% block;
}
...
pub fn applyAncestorLogic(
    ...
    const wx = worldBlock(key.suffix[0], bx);
    const wy = worldBlock(key.suffix[1], by);
