...
/// The two child columns a portal may land on inside its parent's region: the middle pair of
/// `BLOCKS_PER_PARENT`, so the portal always sits under the spot a descent drops the player on
/// (`computeLayer()` centers them in the region horizontally).
...
fn portalChild(
    ...
    if (lx != PORTAL_COLUMN_LEFT + @as(u4, @intCast(parent_hash & 1))) return .{};

    return .{ .id = parent_sprite, .seed = seed, .water_volume = inherited_water };
}

/// A liquid parent's volume as seen by the child at row `ly` of its region, settled to the bottom.
///
/// Every pool has a partly-filled surface cell, and handing its whole `BLOCKS_PER_PARENT`-square region
/// a full volume both multiplies the world's water and leaves the pool out of equilibrium:
/// the sim then corrects it on the chunk's first tick and writes modification entries
/// for water the player never touched.
///
/// Volume is counted in `MAX_HP` units per cell, so one column of the region holds
/// `BLOCKS_PER_PARENT * MAX_HP` of them and the parent contributes `BLOCKS_PER_PARENT` times its own.
/// Filling from the bottom up is what the sim would settle to anyway, so it starts there.
inline fn inheritedLiquidVolume(parent_volume: u4, ly: u4) u4 {
    const max: u32 = memory.Block.MAX_HP;
    const units: u32 = @as(u32, parent_volume) * dw.BLOCKS_PER_PARENT;
    const rows_below: u32 = dw.BLOCKS_PER_PARENT - 1 - ly; // full rows of this column beneath the cell
    return @intCast(@min(max, units -| rows_below * max));
}
...
test "liquid refinement conserves volume and settles downward" {
    const max: u32 = memory.Block.MAX_HP;
    for (0..max + 1) |v| {
        ...
        // One column of the region carries exactly the parent's own volume, `BLOCKS_PER_PARENT` times over.
