...
/// Returns true when a child cell is in the parent core.
/// The core is the only unconditional carve protection.
inline fn isProtectedCell(lx: u4, ly: u4) bool {
    return isParentCore(lx, ly);
}
...
fn carvesSlope(
    ...
    if (isProtectedCell(lx, ly) or protectsDiagonalBridge(n, noise_seed, wx, wy, lx, ly)) return false;
