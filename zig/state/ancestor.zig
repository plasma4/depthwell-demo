...
test "slope carve: face links have no unconditional protection" {
    for (0..dw.BLOCKS_PER_PARENT) |along| {
        ...
        try testing.expect(!isProtectedCell(0, cell));
        try testing.expect(!isProtectedCell(dw.BLOCKS_PER_PARENT - 1, cell));
        try testing.expect(!isProtectedCell(cell, 0));
        try testing.expect(!isProtectedCell(cell, dw.BLOCKS_PER_PARENT - 1));
    }
}
