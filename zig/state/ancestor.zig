...
test "infill: ordinary solids never keep an ore underlay" {
    var clay = Block.makeBasicBlock(.clay, 1);
    clay.base_id = .stone;
    const clay_spec = infillSpec(clay, 2);
    try testing.expectEqual(Sprite.clay, clay_spec.id);
    try testing.expectEqual(Sprite.none, clay_spec.base_id);
