...
pub fn applyAncestorLogic(
    ...
    // A block seed belongs to the cell ADDRESS, not to what occupies it,
    // so EVERY branch below carries this value, air included.
    // A cell that generated air with a seed of zero would hand the block the player builds into it variant 0,
    // then a different variant the first time the chunk regenerates.
