...
fn carvesSlope(
    ...
    // A protected cell needs only one terrain connection. An isolated parent has none to preserve.
    if (isProtectedCell(n, lx, ly)) {
        for (n) |block| if (block.isSolid()) return false;
        return true;
    }
    ...
    // Outside the core, an isolated parent also disappears instead of reaching the density field.
