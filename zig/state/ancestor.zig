...
/// The dial for how much of a lone or thin feature survives beyond its protected core:
/// at `CORNER_UNIT` its outer ring lands on the bare threshold and only a weak erosion mask spares it,
/// and every step up from there widens the fringe.
...
///
/// `quadrant_bit` is that axis' bit of `Coordinate.quadrant`, which is the 65th (and highest) bit of
/// the chunk index; without it the two halves of the world would share a coordinate space and,
/// with it, their noise. Every seed in play is already per-quadrant, so this is belt and braces,
/// but it costs one shift and it is what makes the field unique across the whole world rather than
/// merely across one quadrant of it.
///
/// The product needs 69 bits, which is why noise takes a `WorldCoord` and not a `u64`:
/// see `seeding.foldWorld()` for how it reaches a 64-bit hash without reintroducing a period.
inline fn worldBlock(chunk: u64, quadrant_bit: u1, block: u4) WorldCoord {
...
///
/// This is an invariant of the fractal rather than a tuning knob. Every block the player can stand on
/// at D must still be there at D+1, and the density field alone does not promise that: an isolated
/// block has all four corners at a single solid neighbor (`CORNER_UNIT`), which sits below
/// `SLOPE_THRESHOLD` across its whole region, so a purely density-driven carve deletes the block
/// outright. The same slack lets an unlucky erosion mask hollow a thicker block into a shell.
/// `carvesSlope()` enforces this, and it is the only thing that bounds how much the mask may take.
...
/// Whether a child cell is one the carve may never take: the parent's core, plus an arm reaching the
/// region border toward each SOLID orthogonal neighbor.
///
/// The core alone keeps a block from vanishing but says nothing about the gap between two blocks,
/// and the cells in that gap are the ones least supported by the density field, so they are exactly
/// the ones erosion takes first. A row of blocks at D then arrives at D+1 as a row of detached
/// squares. The arms bridge it: both parents of a solid pair reach the border they share, from
/// opposite sides, so the two cores join into one band `BLOCKS_PER_PARENT / 2` cells thick.
///
/// Both parents read the same pair of blocks to decide, so the two halves of a bridge always agree.
/// Diagonal neighbors get no arm: a corner-only touch is not somewhere the player could have walked
/// at D either.
///
/// Neighbors are row-major with the center removed, so 1 is above, 3 left, 4 right, 6 below.
...
fn carvesSlope(parent_block: Block, n: [8]Block, noise_seed: dw.utils.Vec2u, wx: WorldCoord, wy: WorldCoord, lx: u4, ly: u4) bool {
    ...
    // A thin parent's field is nearly uniform, so it does not erode into a silhouette at all: it sits
    // just under the threshold everywhere at once and loses every cell the guard above does not hold,
    // which is why a lone block came out as a bare square. This gives such a parent its own body back,
    // scaled by how little the neighborhood already supports it, so the noise has something to cut a
    // shape out of. Well-supported terrain is past `BODY_SUPPORT_NONE` and keeps its slopes untouched.
...
pub fn applyAncestorLogic(
    ...
    const wx = worldBlock(key.suffix[0], @intCast(key.quadrant % 2), bx);
    const wy = worldBlock(key.suffix[1], @intCast(key.quadrant / 2), by);
    ...
    if (is_overlay) {
        ...
        // The underlay is what is left once the deposit thins out, so it has to be solid rock.
        // A `.none` underlay (an overlay that never recorded a host, which a placed or base-depth ore
        // can be) would otherwise open air in a cell the carve above just decided to keep, core
        // cells included. Falling back to the parent keeps the local material wherever there is one.
...
test "slope carve: a parent always keeps its core, and only its core is unconditional" {
    // The worst case there is: a lone block with nothing solid around it, so every corner of its
    // region reads one solid neighbor and the density field wants the whole thing gone.
...
test "slope carve: solid neighbors stay joined across the border they share" {
    // A line of blocks at D has to arrive at D+1 as a line, not as a row of detached squares. Every
    // orientation is checked, since the arms are what decide it and each reads a different neighbor.
    const parent: Block = .makeBasicBlock(.stone, 23);
    const solid: Block = .makeBasicBlock(.stone, 24);
    ...
    inline for (pairs) |pair| {
        ...
        try testing.expect(!isProtectedCell(n, if (pair[2]) (if (near_edge) dw.BLOCKS_PER_PARENT - 1 else 0) else CORE_MIN, if (pair[2]) CORE_MIN else (if (near_edge) dw.BLOCKS_PER_PARENT - 1 else 0)));
    }
}
