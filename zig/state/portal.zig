...
const SHARD_RADIUS_MIN: f32 = 8.0;
...
/// Radians a shard sweeps around the portal over its travel, and how much that sweep tightens as the
/// travel runs on (see `drawDebris()`). The tightening is bounded by construction:
...
/// `L` multiplies and `C`/`H` shift, so this brightens the sprite and rotates whatever hue it had
/// toward the mouth's, without flattening every material to one color.
const SHARD_GLOW_LIGHT: f32 = 0.55;
const SHARD_GLOW_CHROMA: f32 = 0.16;
const SHARD_GLOW_HUE: f32 = 0.9;

/// Share of the intake ring thrown back out of the portal rather than drawn in (see `spawnOrbitRing()`).
/// A descent still mostly swallows, but never purely: a minority coming back out is what stops the ring
/// from reading as a flat drain. An ascent leans the other way, the world opening up around the player.
const DESCENT_OUTWARD_RATIO: f32 = 0.28;
const ASCENT_OUTWARD_RATIO: f32 = 0.72;
...
/// LCHA colors for a descent's intake particles: the portal's violets and magentas, plus its gold trim.
/// Sampled uniformly, so the single gold entry keeps that accent to roughly a fifth of the particles.
...
/// The same for an ascent, in the inverted portal's sickly greens.
/// Taken from `sprite_colors.colorsOf(.invportal)`: its four most saturated greens and its gold trim,
/// so a rise reads as that sprite's own light rather than borrowing the portal's violet.
/// Worth re-reading off the generator if the `.invportal` art is repainted.
const INVPORTAL_COLORS = [_]Vec4f32{
    .{ 0.585, 0.113, 2.175, 1.0 },
    .{ 0.675, 0.135, 2.198, 1.0 },
    .{ 0.772, 0.144, 2.060, 1.0 },
    .{ 0.828, 0.181, 2.300, 1.0 },
    .{ 0.792, 0.125, 0.893, 1.0 },
};
...
fn drawDebris(center: Vec2f32) void {
    ...
    for (debris[0..debris_count]) |*shard| {
        ...
        // Terrain arrives as itself and leaves as portal light: over the last stretch of its travel a
        // shard brightens and takes on the mouth's chroma, so the swallowing is something that happens
        // TO it rather than a sprite quietly fading out near the middle.
...
fn spawnSwallow() void {
    ...
    const center = worldToScreen(effect_center);
...
fn spawnIntake() void {
    ...
    if (cap > 2) {
        dw.particles.spawnOrbitRing(worldToScreen(effect_center), effectColors(), .{
            .count = count,
            ...
            // Both directions run a two-way exchange; which way it leans is what reads as the portal
            // swallowing the world (descent) or opening it back up (ascent).
