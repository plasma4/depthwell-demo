...
/// Chunk radius, in D chunks, of the parent region handed to the ancestor cache.
/// The preview spans roughly `SIM_BUFFER_WIDTH / ZOOM_FACTOR` parent chunks, and the generator reads a
/// 6x6 block neighbourhood that can reach one chunk further, so this covers it with room to spare.
const ANCESTOR_SEED_RADIUS: i64 = 4;
...
fn seedAncestorsFromSim(centre: Coordinate, parent_depth: u64) void {
    var dy: i64 = -ANCESTOR_SEED_RADIUS;
    while (dy <= ANCESTOR_SEED_RADIUS) : (dy += 1) {
        var dx: i64 = -ANCESTOR_SEED_RADIUS;
        while (dx <= ANCESTOR_SEED_RADIUS) : (dx += 1) {
