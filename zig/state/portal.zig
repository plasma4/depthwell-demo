...
/// 11x11 is already more than the hot tier holds (`AncestorCache.HOT_SIZE`), so seeding past this
/// would only evict what it just inserted.
const ANCESTOR_SEED_MAX_RADIUS: i64 = 5;

/// Radius in D chunks that covers every parent the preview will read.
///
/// Derived from the window rather than assumed: a zoomed-out camera can make the preview far wider
/// than the `SimBuffer` window, and a radius that fails to reach leaves exactly the cold ancestor walk
/// this seeding exists to avoid. Each D chunk covers `ZOOM_FACTOR` child chunks, plus one for the
/// generator's 6x6 block neighbourhood spilling into the next chunk over, plus one to round up.
...
/// Copies the D chunks the preview reads as parents into the ancestor cache, keyed for the depth the
/// preview generates at.
///
/// `AncestorCache` indexes tiers RELATIVE to `memory.game.depth` (see `relativeTier()`), so installing
/// the transition shifts every existing entry out of reach: a parent at D lands on the tier that holds
/// D-1, misses, and re-derives itself from D-1, which misses in turn, all the way down the chain. That
/// cold walk is what makes the first fill stall instead of being almost free.
///
/// The SimBuffer already holds these chunks fully materialized, but `getCachedChunk()` only consults it
/// when `key.depth == game.depth`, which is false here. Copying them across turns every parent lookup
/// into a hit, so the preview never regenerates a chunk that is already in memory.
///
