...
comptime {
    ...
    // A descent reveals its overlay OVERLAY_ZOOM in, and an ascent hides its live layer OVERLAY_ZOOM
    // short of the end (`coverCurve()`). Past the square root the two thresholds cross, and the ascent
    // would have to finish fading before it started.
...
///
/// An ascent needs this, and a descent does not. The two transitions are mirror images with one
/// asymmetry: whichever layer is zoomed OUT is the expensive one, because its window covers
/// `ZOOM_FACTOR^2` times the chunks at full stretch.
/// - Descending, that is the overlay (D+1). `revealZoom()` already caps it: the overlay stays hidden
///   until the zoom is `OVERLAY_ZOOM` in, by which point its footprint has come down to
///   `(ZOOM_FACTOR / OVERLAY_ZOOM)^2`, and `allocatePreview()` sizes for exactly that.
/// - Ascending, it is the LIVE layer (D), which has no such cap: it stretches all the way to
///   `ZOOM_FACTOR` while the overlay fades in over it.
///
/// So the ascent finishes its fade `OVERLAY_ZOOM` SHORT of the full zoom, mirroring the reveal that
/// holds the descent's overlay back until `OVERLAY_ZOOM` in. Both layers then top out at the same
/// `(ZOOM_FACTOR / OVERLAY_ZOOM)^2` footprint, and the rest of the ascent draws only the overlay
/// (see `liveLayerHidden()`).
fn coverCurve() f64 {
    @setFloatMode(.optimized);
...
pub fn overlayOpacity() f64 {
    ...
    // `OVERLAY_ZOOM < sqrt(ZOOM_FACTOR)` keeps the reveal below the cover; the constant's own bounds
    // check is looser than that, so the ordering is asserted rather than assumed.
...
/// Whether the live layer contributes nothing to this frame, so rasterizing it is pure waste.
///
/// Two ways that happens: a return dive fades the whole world out, and an ascent's overlay goes fully
/// opaque over it at `coverCurve()`. The second is the one that matters, since it is where the live
/// window is widest. The caller must still place the player, who is drawn over both layers.
