...
/// Copied from `.invportal` colors lazily: green hues. Dimmest first, as above.
...
fn spawnIntake() void {
    ...
    // Squared, and then beaten against a wave, rather than emitted at a flat rate.
    //
    // A rate that only ever climbs gives every frame the same texture as the last, so the intake
    // arrives as one undifferentiated mass: the squaring keeps the opening frames genuinely sparse
    // (a few strands, not a thin version of the whole thing), and the wave gives the rest a pulse to
    // read against, gusts hitting the mouth instead of a steady pour. The floor keeps every trough
    // populated, since a gap that empties reads as a dropped frame rather than as a rhythm.
