...
///
/// Deliberately well past `ZOOM_FACTOR`: a descent stops there because that is where its preview lines
/// up, but a return has no preview to meet, so the world simply runs away into the fade. Being a plain
/// multiple of the current view (rather than anything derived from the depth) is what makes the effect
/// identical at depth 40 and at depth 10,000,000.
pub const RETURN_ZOOM: f64 = 24.0;
...
/// The spot inside the inverted portal's own chunk that an ascent carries up: what the dive is aimed
/// at and what the landing one depth higher is derived from. Only meaningful while an ascent runs.
var ascend_origin: dw.utils.Vec2i = .{ 0, 0 };

/// Where the player was standing when the ascent began, which is where a later return brings them
/// back to. Captured at trigger time, before the pull moves them.
...
/// Both directions pull: a descent is swallowed by the portal below, an ascent is drawn up through the
/// inverted one above. Either way the player ends on the very point the layer lands them at, which is
/// what keeps the committing frame from snapping.
...
///
/// 1 except during a return, which fades out over the back of its dive and back in on arrival. The
/// commit lands between the two, on a screen that is already empty, so the depth change is never seen.
...
fn ensureReady() void {
    ...
    if (isReturning()) {
        // Nothing left to prepare once the return has committed: the world is already at D+1 and the
        // remaining frames only fade it in. `retraceStep()` is gone by then too (it was popped at the
        // commit), so re-deriving the transition would read the wrong step.
        if (memory.game.portal_frame >= RETURN_DIVE_FRAMES) {
            ready = true;
            return;
        }
        // A return dives back to a depth already visited, at a spot already recorded, so its D+1
        // transition is read straight back rather than recomputed (see `world.computeRetraceLayer()`).
        transition = world.computeRetraceLayer(world.retraceStep().?);
        computeEffectCentre(bx, by);
        anchor = effect_centre;
        allocatePreview();
        collectDebris();
        ready = true;
        return;
    ...
    if (isDescending()) {
        ...
        // The inverted portal is what the player rises through, exactly as the portal is what they
        // fall into, so the landing is pinned to that block rather than to wherever they stood.
        // Deriving it from the player instead would drop them at an unrelated spot on arrival, which
        // is the whole reason the descent uses the block too.
    ...
    planFill();
    allocatePreview();
    // Both directions spread generation across the animation now (budgeted per frame by `tick()`),
    // so neither builds the whole preview here: doing so was a several-hundred-millisecond spike in
    // Debug the moment the indicator was clicked.
    collectDebris();
    ready = true;
}
...
/// Places `effect_centre` on the portal block's centre, in subpixels relative to the player's chunk.
...
fn fillPreview(count: u32) void {
    ...
    // The D parents the deeper preview reads live around the player's current chunk (a descent's
    // portal, and a return's landing, are both within a few blocks of it).
    ...
    // Going-deeper only: it warms the D parents the D+1 preview reads, which are resident in the
    // SimBuffer. An ascent reads D-2 parents (shallower than the live D world), which are not resident
    // and would key into a negative tier relative to the installed D-1 depth, so it is skipped.
...
///
/// Eased onto the anchor as the player is drawn in, so the zoom closes on the portal they are passing
/// through rather than on wherever the camera was loitering. `camera_pos` itself is never written,
...
/// Commits a return under the dark frame: finishes any preview still owed, installs D+1 and adopts the
/// generated chunks, so the arrival fades in on a ready world rather than one being generated.
fn commitReturn() void {
    // Anything the dive did not reach (a stall, a low frame rate) is finished now. Bounded by the
    // preview size, and the screen is already black, so a small catch-up here is invisible.
...
fn spawnReturnIntake() void {
    ...
    // past that the dive is running away into the fade and an honestly-scaled ring would spend its
    // particles far off screen.
...
fn finish() void {
    ...
    if (ascended) {
        ...
        // The invportal is only the way up: a later return puts the player back where they were
        // STANDING when they used it, not inside the block they rose through.
    ...
    // Hand the generated chunks to the SimBuffer before dropping them, both directions: skipping it
    // does not avoid the work, only defers a full 256-chunk rebuild to the next `player.move()`.
    world.SimBuffer.refreshAdopting(g.getPlayerCoord(), PreviewSource{});

    // An ascent's preview was generated before `applyAscent()` recorded the markers (and, on a first
    // ascent, before the world spectated at all), so re-apply them onto the adopted chunks. Cheap:
    // a no-op unless something was actually modified below (see `applyDescendantMarkersToSim()`).
...
///
/// A return saved AFTER its commit has no preview to rebuild (`ensureReady()` no-ops there): the world
/// is already at D+1 and only the fade-in remains.
pub fn restore() void {
    ready = false;
    if (!isActive()) {
        releasePreview();
    ...
    ensureReady();
    // Loads can be slow, so the whole preview is fine to build at once here. The `preview.len` guard
    // covers a post-commit return, whose `ensureReady()` allocated nothing.
