...
/// 0 means the last import succeeded. This mapping is mirrored by `IMPORT_ERROR_LABELS`
/// in `saveManager.ts`; keep the two in sync when adding a `SaveError`.
...
/// The category code of the most recent import failure (0 = none); read by the host after a failed load.
