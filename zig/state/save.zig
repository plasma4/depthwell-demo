...
fn readQuadCache(r: *Reader) !void {
    ...
    qc.left_path.clearRetainingCapacity();
    try qc.left_path.growCapacity(world.alloc, path_len);
    for (0..path_len) |_| try qc.left_path.append(world.alloc, try r.int(u64));

    try qc.top_path.growCapacity(world.alloc, path_len);
    qc.top_path.clearRetainingCapacity();
    for (0..path_len) |_| try qc.top_path.append(world.alloc, try r.int(u64));
}
