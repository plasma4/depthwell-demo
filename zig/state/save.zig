...
fn writeEntryPayload(w: *Writer, entry: *const world.ModEntry) !void {
    // Both fields have a fixed native layout on the supported little-endian
    // targets.  Emit them as contiguous slices so ArrayList only has to do
    // one append for each field instead of one append per word/cell.
...
/// This lets callers reserve the final output space before the high-volume
/// entry writes begin.
...
fn serialize(w: *Writer) !void {
    ...
    // Reserve the exact large section plus the end marker and checksum. This
    // avoids repeated ArrayList growth/copying while modified cells are added.
