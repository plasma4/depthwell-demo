...
/// v4 drops `ancestor_materials` and stores `materials_path` as `HorizonTrace` rather than whole
/// windows: a window is derived state, so saving one both bloats the blob by 4 KiB a depth and freezes
/// whatever the refinement produced at the time, bugs included (see `QuadCache.getMaterials()`).
const QUADCACHE_VERSION = 4;
...
fn readQuadCache(r: *Reader, section_version: u16) !void {
    // Every version so far changed the SHAPE of this section's payload, not just its meaning, and the
    // framing length keeps the stream aligned either way: a blind read would not fail, it would fill the
    // descent state with whatever followed. Refuse instead.
