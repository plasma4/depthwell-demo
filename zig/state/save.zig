...
//!
//! MOD_STORE holds only the cells the player authored, not whole chunks: everything else in a chunk is
//! regenerated on load (see `world.materializeChunk()`). A record is therefore variable-length.
//!
...
//!   whenever the game mutates a planned entry before it has been encoded, `shadowEntryForSave()` first
//!   serializes that entry's start-of-snapshot payload into a side map, and the batch encodes from the shadow.
...
test "MOD_STORE: an entry round-trips through its record encoding" {
    world.mod_store.init(testing.allocator);
    defer world.mod_store.deinit();

    const key: DepthCoordinate = .{ .suffix = .{ 0xDEAD, 0xBEEF }, .depth = 11, .quadrant = 2 };
