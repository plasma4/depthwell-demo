...
pub fn finalizeLoad() void {
    ...
    // quadrant at HORIZON_DEPTH. The same suffix in quadrant 0 generates identically, since no seed
    // at those depths reads the quadrant, so this lands the player on the terrain they left.
