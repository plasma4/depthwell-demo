...
pub fn finalizeLoad() void {
    ...
    // Resolved here rather than left for the first tick: a frame renders before that tick, and it
    // would draw every one of those chunks with the stale flags for its duration.
