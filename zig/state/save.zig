...
test "mod_store: encoding/decoding is correct" {
    ...
    // A real load builds `id_remap` from the SPRITE_TABLE section first (`remapSpriteId()` returns
    // `.none` for any unlisted id), so register identity mappings for the sprites this fixture uses.
