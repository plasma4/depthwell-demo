...
/// Every row's number there depends on the rows above it.
/// Inserting one drop means rewriting the rest and hoping the last branch still totals 1.0.
...
/// (Done with Lemire's method, the same one `HashState.getLimit()` uses.)
///
/// Costs a relative bias under `total / 2^32`, far under the bias already in `FastHash`.
/// Then it walks the running weight totals, which are comptime constants.
/// After compiling, the whole pick becomes one multiply and a chain of compares against literals.
