...
/// A world block coordinate on ONE axis, in wrapping 64-bit arithmetic.
///
/// A block index at the horizon depth needs 69 bits, so this type does NOT hold every position
/// exactly: the top bits wrap. Wrapping is the deliberate choice over a `u128`,
/// which cost ~46% more per noise sample on wasm32 (two 128-bit multiplies sit in the innermost
/// loop of every noise function, and those functions are ~40% of chunk generation).
/// What wrapping costs is a repeat, plus one seam where the field is discontinuous rather than
/// merely shifted. Both sit at the same distance: ~2^28 blocks at the very finest lattice scale
/// (see `LATTICE_FRAC_BITS`) and far larger at every other scale, against a screen 30 blocks wide.
/// Every local difference below that distance is exact,
/// so terrain is continuous across every chunk border a player can actually stand near.
///
/// Depth and quadrant are separated by the SEED rather than by the coordinate
/// (see `applyAncestorLogic()`), so the wrap cannot make two different depths or quadrants agree.
pub const WorldCoord = u64;
