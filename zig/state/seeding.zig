...
pub const FastHash = struct {
    ...
    /// Vectorized 4-lane version of `hash2d()`, hashing 4 coordinates in parallel (using SIMD).
    /// Each lane `i` equals `hash2d(seed_vector, vx[i], vy[i])` exactly
    /// (enforced by the "hash2d_4x matches hash2d" test).
