...
///
/// It also fixes where the coordinate wraps: the scaled product is a `WorldCoord`,
/// so the lattice repeats every `2^(64 - LATTICE_FRAC_BITS) / inv_scale` blocks.
/// See `WorldCoord` for why a repeat that distant is not worth a wider integer.
const LATTICE_FRAC_BITS = 32;
/// Largest lattice frequency, in cells per block, the fixed-point placement can represent.
/// Bounds the scale factor at `2^(LATTICE_FRAC_BITS + 4)`, so `step` fits a `u64`
/// with plenty of headroom and the wrap stays as far out as the comment above claims.
const MAX_INV_SCALE = 16.0;

/// One axis of a world coordinate placed on a noise lattice.
const LatticeAxis = struct {
    /// Index of the cell the coordinate falls in.
    cell: u64,
...
///
/// Fixed point rather than `f64`: a coordinate runs to 64 bits and an `f64` product quantizes past
/// 2^53, which would freeze the field into one value per step across most of the world.
...
inline fn oreField(seed: Vec2u, x: WorldCoord, y: WorldCoord, lane: u3, rule: OreDispersal) f32 {
    ...
    const sample_x = x +% @as(WorldCoord, @bitCast(warp_x));
    const sample_y = y +% @as(WorldCoord, @bitCast(warp_y));
...
test "noise resolves far-out coordinates block by block" {
    const seed: Vec2u = .{ 0x243f6a8885a308d3, 0x13198a2e03707344 };
    const scale = 1.0 / 7.0;

    // Deep in the world, where an f64 product would quantize to one value per step and the old
    // 32-bit mask made the whole field repeat.
    const origin: WorldCoord = 1 << 52 | 12345;
    for ([_]WorldCoord{ 1 << 32, 1 << 40, 1 << 51 }) |period| {
...
test "lattice cells stay adjacent everywhere short of the wrap" {
    // Cells must share corners, or the terrain tears along a line: the corner one cell up must equal
    // the next cell's own corner. This holds for every coordinate below the wrap, and the wrap is
    // one seam the whole world over, at a distance nobody reaches (see `seeding.WorldCoord`).
    const scale = 1.0 / 4.0; // 4 blocks per cell, so a cell boundary is easy to straddle
    const step: u64 = @intFromFloat(@round(scale * (1 << LATTICE_FRAC_BITS)));
    const seam = @divTrunc(std.math.maxInt(u64), step) & ~@as(u64, 3); // first block whose product wraps
    try std.testing.expect(seam > 1 << 30); // the seam is far outside any reachable coordinate

    for ([_]WorldCoord{ 0, 12345 << 20, seam - 16, seam - 8 }) |v| {
    ...
    const quarter = latticeAxis(1 + (12345 << 20), scale);
