...
/// Rescales `F2 - F1` so the field keeps the SPREAD the terrain thresholds were tuned against.
///
/// A 2x2 search used to miss the true second-nearest feature point often, which inflated `F2 - F1`;
/// searching all nine cells finds it, and the raw field came out visibly tighter (the world went from
/// 43% to 31% solid on an unchanged seed). Every density threshold in `generateBaseProceduralSprite()`
/// and `ORE_DISPERSALS` reads this field, so the honest fix is one factor here rather than
/// re-tuning several dozen constants against a distribution that only moved.
const WORLEY_NORM: f32 = 1.0;
...
fn worleyValue(
    ...
    return raw + WORLEY_NORM * raw * (1.0 - raw);
}
