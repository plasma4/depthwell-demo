...
fn computeBaseSpriteType(
    ...
    // NOTE: `use_f2_f1` defaults to TRUE, so this takes the domain-warped Worley path, not the cheap Perlin
    // one. That makes moisture a genuinely per-block field despite its 425-block cells: it cannot be sampled
    // on a coarse lattice and interpolated (measured: 0.6% of blocks change sprite on an 8-block lattice).
