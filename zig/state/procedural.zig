...
pub fn disperseOre(host: Sprite, density: f32, x: u64, y: u64, depth: u64, seed: Vec2u) ?Sprite {
    ...
    inline for (ORE_DISPERSALS) |rule| {
        ...
        if (density < rule.min_density or density > rule.max_density) continue;
        if (oreField(oreSeed(seed, rule.seed_lane, depth), x, y, rule) >= rule.threshold) return rule.sprite;
    }
    return null;
}
