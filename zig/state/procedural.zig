...
/// Blocks per cell of the domain warp that hides the density field's Worley grid.
///
/// Bounded from BELOW by folding, not by looks. The warp displaces a sample by up to ~40 blocks,
/// so a cell size near that displacement makes the warped position move BACKWARD as the real
/// coordinate moves forward, and the field then mirrors a feature onto itself a few blocks away.
/// At 21 that happened on 11.5% of block steps; 30 halves it for the same displacement.
pub const dual_value_scale = TuningFloat(30.0);
...
///
/// The two INTERPOLATED corners are precomputed, because every value-noise and Perlin sample reads
/// both and they far outnumber everything else here. The cell itself is kept alongside them purely so
/// the Worley search can step BACKWARD (see `WorleyAxis`); computing that corner eagerly cost the
/// two-corner paths about a third of their time at recursive depths, for a corner they never read.
const LatticeAxis = struct {
    /// Hash inputs for the cell the sample is in and the one after it, already folded to 64 bits.
    corners: [2]u64,
    /// Low half of the cell the sample is in. A cell index needs up to 73 bits, so the rest of it
    /// lives in `folded` rather than as a second integer.
    cell_low: u64,
    /// The cell's high half, ALREADY multiplied by `seeding.FOLD_MULTIPLIER`.
    /// Zero for every coordinate short of 2^32, which is the whole base depth.
    folded: u64,
    /// Position within the cell, in [0, 1).
    t: f32,

    /// Hash input for the cell `offset` steps along this axis, folded to 64 bits.
    /// Equals `seeding.foldWorld(cell + offset)` exactly.
    ///
    /// Stepping the low half over its own boundary carries into the high half, and a fold of one more
    /// high unit is exactly one more `FOLD_MULTIPLIER`, so a step is an add rather than a second multiply.
...
/// One warped axis of a Worley sample: the three candidate cells and where the sample sits in the middle one.
///
/// THREE, not two, and this is the whole reason the field is continuous.
/// A feature point one cell BEHIND the sample can easily be the nearest one, so a window of
/// `{cell, cell + 1}` drops the current winner the instant `t` wraps past a cell boundary,
/// and `d2 - d1` jumps with it. Adjacent blocks used to differ by as much as 0.99 that way,
/// which put hard straight seams through the density field (and so through the terrain).
const WorleyAxis = struct {
    /// Hash inputs for the cells at offsets -1, 0, and +1 from the sample's own cell.
    corners: [3]u64,
...
/// Restores the SPREAD of `F2 - F1` that the terrain thresholds were tuned against.
///
/// A 2x2 search used to miss the true second-nearest feature point often, which inflated `F2 - F1`;
/// searching all nine cells finds it, and the raw field came out visibly tighter (an unchanged seed
/// went from 43% to 31% solid). Every density threshold in `generateBaseProceduralSprite()` and
/// `ORE_DISPERSALS` reads this field, so one factor here is the honest fix rather than re-tuning
/// several dozen constants against a distribution that only moved.
///
/// 0.43 is where the solid share of a 921,600-block sweep returns to what it was (43.1% -> 43.3%).
const WORLEY_SPREAD: f32 = 0.43;

/// Cell offsets, on one axis, that the Worley search covers.
const WORLEY_OFFSETS = [_]comptime_int{ -1, 0, 1 };
/// Candidate cells one Worley sample tests. Only the first `WORLEY_TAPS - 1` are hashed in vectors.
...
fn worleyValue(
    ...
    // The 3x3 grid search. `latticeAxis()` already folded every corner, so these are hash inputs
    // rather than cell indices. Eight of the nine are hashed as two vectors and the last on its own,
    // which is one fewer hash than padding a third vector out.
    ...
    inline for (0..WORLEY_TAPS) |i| {
        const off_x = @as(f32, @floatFromInt(@as(u32, @truncate(hashes[i])))) * INV_POW_2_32;
        const off_y = @as(f32, @floatFromInt(@as(u32, @truncate(hashes[i] >> 32)))) * INV_POW_2_32;
    ...
    // A monotone bulge rather than a plain multiply: scaling up alone piles the top of the field onto
    // the 1.0 clamp, and `density >= density_max` reads that as AIR, so a louder scale carved the world
    // open instead of filling it. This fixes both endpoints and only stretches the middle.
    return raw + WORLEY_SPREAD * raw * (1.0 - raw);
}
