...
/// `latticeAxis()` keeps only bits `SPLIT_BIT`..`SPLIT_BIT + 64`,
/// so a wrapped sample lands in an unrelated fold band and seams the field.
/// Mirroring folds the offset back instead, which keeps the field continuous
/// (the value at `0 - n` becomes the value at `n`) and touches nothing
/// more than `MAX_WARP_BLOCKS` from the origin.
