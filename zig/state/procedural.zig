...
///
/// Keep this OFF the chunk grid. Value noise is pinned at its lattice points (the fade curve zeroes the
/// derivative there), so at 16 -- exactly `CHUNK_SIZE` -- the warp stamped a 16-block phase onto the terrain:
/// a flat 8-column run was ~2x more likely at `x % 16 >= 10` than below it, which biased where structures
/// could seat. A non-power-of-two decorrelates it (see `debug/audit.zig`).
