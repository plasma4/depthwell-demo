...
pub fn clearCaches(comptime clear_ancestors: bool) void {
    SimBuffer.clear();
    chunk_cache.clear();
    // The mining light window is memoized per logic tick and anchored on the player's block, so a
    // teleport or depth change (both of which land here) can move it mid-tick without the tick counter
    // noticing. Dropping it with everything else keeps it from outliving the world it was flooded over.
