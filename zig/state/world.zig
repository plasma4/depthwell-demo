...
pub fn computeLayer(coord: Coordinate, bx: u4, by: u4, anchor: LayerAnchor) LayerTransition {
    ...
    if (new_depth <= HORIZON_DEPTH) {
        ...
        if (chunk_offset[1] != 0) {
            target_coord = target_coord.moveAtDepth(chunk_offset, new_depth) orelse target_coord;
        }
    ...
    // 32 * 2 - 2 = 62; shift 62 bits so the top bits provide a value between 0 and ZOOM_FACTOR - 1
    ...
    const target_horizon_depth = new_depth - dw.HORIZON_DEPTH;
    if (target_horizon_depth >= STARTING_ZOOM_TIMES) {
        const trace = traceHorizon(target_coord, new_pos, new_depth, @intCast(coord.quadrant));
