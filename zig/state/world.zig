...
pub const QuadCache = struct {
    ...
    pub inline fn getQuadrantSeed(self: *const @This(), quadrant: u2, depth: u64) seeding.Seed {
        ...
        // Asked FIRST, and not after the current-depth branch. `path_hashes` is only stepped past the
        // horizon, so at HORIZON_DEPTH itself it still holds whatever the deepest descent left in it:
        // an ascent from HORIZON_DEPTH + 1 does not clear it, and a fresh world has not written it yet.
...
///
/// The world at D+1 is 4 cells per axis wide when D is `HORIZON_DEPTH` and 8 when D is deeper,
/// which is more than one window can name, so a descent has to choose which 2 to keep:
/// that choice is the rebase, and `maxOriginCell()` is how far the choice reaches.
///
...
pub const LayerTransition = struct {
    ...
    top_cell: u3 = 0,
    /// Whether the window has sat against that edge of the world at EVERY rebase down to `depth`.
    /// One false anywhere on the path means the world continues that way (see `replayRebaseState()`).
    most_top: bool = true,
    most_bottom: bool = true,
    most_left: bool = true,
    most_right: bool = true,
...
///
/// Everything a rebase touches is either one global, one slot of a rolling buffer,
/// or one entry of a path list, so a whole undo record is this small.
pub fn snapshotLayer(next_depth: u64) LayerSnapshot {
    const ring: usize = @intCast(next_depth % QuadCache.HISTORY_LEN);
    // The two path lists are written together and only ever by `writeRebasePath()`.
...
/// so two descents that land on the same cell seed it the same way.
/// Being a pure function of `(parent, depth, left_cell, top_cell)` is also what lets
/// `replayRebaseState()` rebuild any depth's seeds from the recorded origin path,
/// instead of from `quad_cache.historical_seeds`, whose ring aliases once an ascent runs
/// deeper than `HISTORY_LEN`.
...
/// Last cell the rebase window's left (or top) cell can be at `depth`. Asserts `depth > HORIZON_DEPTH`.
///
/// A window names 2 cells per axis, and a depth change splits every cell of the parent into
/// `ZOOM_FACTOR`. The parent of the first rebase is `HORIZON_DEPTH`, which is ONE cell wide
/// (4**32 chunks is exactly one suffix), so its children span 4 cells and the window's left cell
/// runs 0 to 2. Every deeper parent is a 2-cell window, so its children span 8 and the range is 0 to 6.
///
/// Without the first-rebase case a descent could put the window on cell 4 at `HORIZON_DEPTH + 1`,
/// which has no parent chunk at all: it reads back as a duplicate of cell 0 and hides the world edge.
...
fn replayRebaseState(depth: u64) RebaseState {
    ...
    while (d <= depth) : (d += 1) {
        ...
        const max_cell = maxOriginCell(d);
        state.hashes = stepQuadrantSeeds(state.hashes, d, left_cell, top_cell);
        // The same and-folds `computeLayer()` step 4 does on the way down, in the same order.
        state.edges.most_left = state.edges.most_left and left_cell == 0;
        state.edges.most_right = state.edges.most_right and left_cell == max_cell;
        state.edges.most_top = state.edges.most_top and top_cell == 0;
        state.edges.most_bottom = state.edges.most_bottom and top_cell == max_cell;
    }
    return state;
}
...
pub fn computeLayer(coord: Coordinate, bx: u4, by: u4, anchor: LayerAnchor) LayerTransition {
    ...
    // The landing chunk. At or before the horizon the suffix still has room for the new path step,
    // so the quadrant carries over untouched; a rebase past the horizon replaces it below.
    var landing: Coordinate = .{ .suffix = landing_suffix, .quadrant = coord.quadrant };

    if (new_depth > HORIZON_DEPTH) {
        // Past the horizon the suffix is full, so the top path step it just lost has to go somewhere.
        // A chunk address at D is quadrant * 2**64 + suffix, which needs 65 bits.
        // Zooming multiplies it by ZOOM_FACTOR and adds the child index, which needs 67.
        // A window names 2 cells, or 65 bits, so a descent has to choose WHICH 2 of the parent's child cells to keep.
        // That 3-bit choice per axis is the rebase, and it is what the path lists woring, forever!
        t.rebase = true;

        // Bit position of the suffix's top path step: HORIZON_DEPTH steps of ZOOM_LOG2 bits,
        // minus the one step being read. 32 * 2 - 2 = 62.
        ...
        // Step 1: figure out the cell the landing chunk falls in. The source quadrant picks one group of ZOOM_FACTOR cells,
        // and the top path step of the source suffix picks one cell inside it.
        ...
        // Step 2: which half of that cell it falls in. Everything scales by the same ZOOM_FACTOR,
        // so the source's own low suffix bits answer this before the zoom.
        ...
        // Step 3: place the window so the landing chunk sits near its center. Take the cell to the
        // left when the chunk is in the left half of its own cell, and the cell above when it is in
        // the top half. The chunk then ends up within half a cell of the window center, which leaves
        // at least 2**63 chunks of travel in every direction before an address runs out.
        // The saturating subtract and the clamp hold the window inside the world. They only fire at
        ...
        // The landing has to be one of the 2 cells the window kept, or its quadrant would not fit in
        // 2 bits. It always is: the clamp above only moves the window when the landing is against the
        // world edge, which is the side the window is already on.
        std.debug.assert(landing_cell_x <= @as(u4, max_cell) + 1);
        std.debug.assert(landing_cell_y <= @as(u4, max_cell) + 1);

        // Step 4: world edges. The window still touches the left edge of the world only if EVERY
        // rebase so far kept it against cell 0, so these are and-folds down the whole descent path.
        quad_cache.most_left = quad_cache.most_left and left_cell_x == 0;
        quad_cache.most_right = quad_cache.most_right and left_cell_x == max_cell;
        quad_cache.most_top = quad_cache.most_top and top_cell_y == 0;
        quad_cache.most_bottom = quad_cache.most_bottom and top_cell_y == max_cell;
        ...
        // Step 5: reseed the four quadrants from the cells they were split out of. HORIZON_DEPTH is
        // one cell wide and has no rebased parent to read, so the first rebase starts from the world
        // seed: `maxOriginCell()` keeps its window inside that one cell, so only lane 0 is ever read.
        const old_hashes: ChunkSeeds = if (new_depth == HORIZON_DEPTH + 1) blk: {
            // The window stays inside the single parent cell, so `stepQuadrantSeeds()` reads lane 0
            // for all four quadrants and the other three lanes never matter.
        ...
        // Step 6: record the window origin. The path lists keep it forever,
        // and the rolling buffers answer for the depths that are still live (see `QuadCache.getOriginX()`).
        ...
        // The landing's quadrant is its place inside the window, 0 or 1 per axis.
        landing.quadrant = @intCast((landing_cell_x - left_cell_x) + (landing_cell_y - top_cell_y) * 2);
    }

    // The pivot can push the player out of the bottom of their chunk, which moves the landing into the chunk under it.
    // At the world's floor there is no such chunk,
    // so the player stands on the bottom of the one they are in instead of jumping a whole chunk back up.
    if (chunk_offset[1] != 0) {
    ...
    // The horizon. H sits HORIZON_DEPTH shallower than the depth being entered, so it moved one depth deeper too,
    // and its window is one refinement step from the window before it.
    // Nothing to refine until H reaches the first generated depth.
...
test "rebase: the first window cannot name a cell the horizon depth does not have" {
    // HORIZON_DEPTH is exactly one suffix wide, so the depth under it is 4 cells and a 2-cell window
    // starts at 2 at the latest. A window at 3 would cover cell 4, which has no parent chunk at all:
    ...
    try testing.expect(right.most_right and right.most_bottom);
    try testing.expect(!right.most_left and !right.most_top);
    ...
    try testing.expect(origin.most_left and origin.most_top);
    try testing.expect(!origin.most_right and !origin.most_bottom);
}

test "moveAtDepth: the horizon depth ends at its suffix, having no quadrant bit to spend" {
    // The world at HORIZON_DEPTH is 4**32 chunks, which is one full suffix. Letting the quadrant bit
    // carry there would hand back an address 2**64 chunks away that generates as a copy of this one.
    const max = std.math.maxInt(u64);
...
test "a preview install puts every global back" {
    ...
    // Anything `installLayer()` writes that `snapshotLayer()` forgets leaks the deeper depth into the
    // live world, where it reads as terrain that regenerates differently on the very next frame.
    const saved_game = memory.game;
    const saved_suffix = max_possible_suffix;
    const saved_path_len = quad_cache.left_path.len;
    ...
    quad_cache.most_top = true;
    quad_cache.most_bottom = false;
    quad_cache.most_left = true;
    quad_cache.most_right = false;
    ...
    try testing.expect(quad_cache.most_top and !quad_cache.most_bottom and quad_cache.most_left and !quad_cache.most_right);
