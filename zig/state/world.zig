...
pub fn applyAscent(t: LayerTransition) void {
    ...
    // The block the retracing descent must aim at, in the parent's coordinates, plus the spot at this
    // depth to put the player back on. Captured before `commitLayer()` moves them.
    ascent_stack.append(memory.main_allocator, .{
        ...
        .origin_pos = memory.game.player_pos,
    }) catch memory.oom();
...
test "AscentStep: retracing the recorded block returns to the chunk that was left" {
    ...
    // Returning puts the player back exactly where they left, not on the target block's floor.
