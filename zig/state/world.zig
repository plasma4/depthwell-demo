...
pub const ModEntry = struct {
    ...
    ///
    /// Carries no cell value of its own: it only says "something was changed below here", which
    /// `materializeChunk()` turns into `Block.descendant_mods` while the player is spectating.
...
pub const ModificationStore = struct {
    ...
    pub fn beginWrite(self: *@This(), key: DepthCoordinate) ModWriter {
        // Spectating layers are read-only, and every gameplay mutation path is supposed to be gated
        // before it gets here (see `isSpectating()`). This is the backstop that catches a missed one:
        // a write above the deepest visited depth would be a modification the retraced descent has no
        // frame to put back.
...
/// so coming back down anywhere else would renumber every suffix at that depth and orphan every
/// `mod_store` key below it. Keeping the block means the way down is the way you came.
pub const AscentStep = struct {
    /// The chunk at the DEEPER depth to come back to, and where in it the player stood.
    ///
    /// Recorded rather than recomputed on purpose. A retrace must not re-derive the depth's rebase
    /// origins: those were fixed by the descent that first reached it, and re-deriving them from
    /// wherever the player happens to have wandered can land on different ones, which renumbers every
    /// suffix at that depth and orphans its `mod_store` keys. `computeRetraceLayer()` reads the
    /// recorded frame back instead, exactly as `computeParentLayer()` does going the other way.
    suffix: Vec2u,
    quadrant: u2,
...
///
/// Lives on `main_allocator` rather than the world arena for the same reason `mod_store` does:
/// it is pushed and popped over a world's life rather than reset with it.
...
///
/// A spectating layer is strictly READ-ONLY. Nothing may write `mod_store` (`beginWrite()` asserts
/// this), the water sim is held, menus that mutate are shut, and the only descent allowed is a
/// retrace of `ascent_stack`'s last step. That is what lets a deep world be revisited from above
/// without any depth's coordinate frame ever being recomputed.
...
pub const MaterialsMode = enum {
    ...
    ///
    /// 128x less memory (4096 / 32), paid for in time: a rebuild is O(depth) refinements, once per
    /// ascent and never per frame. Measured at ~3.2 us a refinement in ReleaseFast, so roughly
    /// 3 ms at depth 1000, 30 ms at depth 10k, 0.3 s at depth 100k and half a minute at depth 10M.
    /// That moves the depth ceiling out by orders of magnitude rather than removing it.
    derived,
};
...
pub const QuadCache = struct {
    ...
    /// First depth whose `ancestor_materials` is recorded, matching the depth `computeLayer()` starts
    /// building the grid at (`target_horizon_depth >= STARTING_ZOOM_TIMES`).
...
pub fn installLayer(t: LayerTransition) void {
    ...
    if (t.has_materials) {
        ...
        // Recorded here rather than in `computeLayer()`: the grid is refined downward and cannot be
...
/// rebuild any depth's seeds from the recorded origin path instead of from
/// `quad_cache.historical_seeds`, whose ring aliases once an ascent runs deeper than `HISTORY_LEN`.
...
/// The rebase state at `depth`, rebuilt from the recorded origin path rather than read back from the
/// rolling buffers.
///
/// Both fields here are accumulations down the path: the seeds chain through `stepQuadrantSeeds()`,
/// and the edge flags are `and`-folds. Neither can be undone a step at a time, so an ascent replays
/// them from `HORIZON_DEPTH + 1`. `left_path`/`top_path` keep every origin ever taken, so this is exact.
...
///
/// Records the step on `ascent_stack`, which both puts the world into its read-only spectating mode
...
pub fn popLayer() void {
    const g = &memory.game;
    // The hotkey has no portal to rise through, so the player's own spot is both the point carried
    // up and the one to come back to.
...
///
/// The exact mirror of `computeParentLayer()`: every field is READ BACK rather than recomputed. The
/// depth's coordinate frame was fixed by the descent that first reached it, and a return must not
/// disturb it, so the rebase origins come from `left_path`/`top_path`, the seeds from replaying those,
...
pub fn retraceInstant() void {
    // No preview warmed the ancestor cache here (unlike the animated return), and its entries are
    ...
    // `commitLayer()` empties the `SimBuffer` and nothing else refills it this tick, so every render
    // frame until the next move would regenerate the visible window instead. One full rebuild now is
    // cheaper than that, and the hotkey is a debug convenience where a one-frame spike is acceptable.
    // Must follow the pop, so the markers baked in match whether the depth landed on still spectates.
    SimBuffer.sync(memory.game.getPlayerCoord(), .{ 0, 0 });
}

/// Commits an already-computed return transition, popping the ascent stack.
///
/// The animated counterpart to `retraceInstant()`: the return spent its dive generating the D+1
/// preview, so the caller adopts that rather than rebuilding, and the ancestor cache it warmed is
/// kept. Does NOT touch the `SimBuffer`; the caller hands the preview over.
...
///
/// An ascent adopts a preview generated before its markers existed (or, on a first ascent, before the
/// world was spectating at all), so the adopted chunks can be missing the bit. This re-applies it
/// without regenerating anything. Cheap by construction: only chunks that carry a descendants-bearing
/// entry do any work, and with no modifications there are none, so it is a single empty-map check.
...
///
/// Only ever one level: an entry at `child_depth` already summarises everything below it, because
/// this ran when that depth was itself ascended past. That induction is what keeps a 300-deep world
/// from needing a 300-deep walk, and it is also why depth must never skip a level.
fn markDescendantsFromChild(child_depth: u64) void {
    ...
    // Collect the child keys first: `markDescendant()` inserts parent entries into `mod_store.index`,
    // which would invalidate an iterator held over it. The keys are a small copy; the entry pointers
    // stay valid across the inserts because `entries` is a pointer-stable `SegmentedList`.
...
test "AscentStep: a return reads the frame back rather than recomputing it" {
    // The whole basis of the read-only spy-glass. A depth's rebase origins are fixed by the descent
    // that first reached it; if a return recomputed them from wherever the player wandered to, it
    // could land on different ones, renumbering every suffix at that depth and orphaning its
    // `mod_store` keys. So `computeRetraceLayer()` reads them back and touches nothing.
    const saved_game = memory.game;
    const saved_suffix = max_possible_suffix;
    defer {
        memory.game = saved_game;
