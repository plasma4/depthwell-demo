...
///
/// `procedural`'s own cache memoizes only the raw terrain sample; the ore and structure passes layered on top
/// of it are the expensive part, and they were being redone every time a block got re-resolved. Blocks get
/// re-resolved constantly: `addEdgeFlags()` rebuilds a one-block halo by re-deriving its neighbors from
/// scratch, so every chunk re-derives the border blocks of the four chunks around it -- blocks those
/// neighbors already derived, or are about to.
const FOUNDATION_CACHE_SLOTS = 8192;
var foundation_cache: [FOUNDATION_CACHE_SLOTS]FoundationCacheEntry = @splat(.{});
/// Terrain identity the cache holds; a mismatch (reseed, or a dragged debug slider) drops every entry.
...
/// - `light`/`lighting_color` are written only into the per-frame render scratch buffer (`applyLighting()`).
/// - `edge_flags`/`id_edge_flags`/`waterlogged` are recomputed from neighbor `id`+`hp` by the flag passes.
/// - `group_x`/`group_y` are stamped during generation and cleared by any edit.
...
/// A chunk the player mined 30 blocks out of costs ~210 bytes here instead of 4 KiB.
///
/// Mutate ONLY through `ModificationStore.beginWrite()`: it shadows the entry for an in-flight save first,
/// and `setCellRaw()` is file-private so no other module can bypass that.
...
///
/// Lives on `memory.main_allocator`, NOT the world arena: entries are freed and their cell blocks
/// recycled, which an arena cannot do.
pub const ModificationStore = struct {
    ...
    /// Every modification entry. `SegmentedList` only ever appends, so an index (and a pointer) into it
    /// stays valid across later insertions: `save.zig` freezes a plan of `entries` indices and resolves
    /// them frames later, and an entry can be mutated while another is created.
    ...
    ///
    /// This is the ONLY way to mutate the store: it preserves the entry's pre-edit contents for an
    /// in-flight budgeted save before handing back a writer, so no call site can forget to.
    /// Costs one predictable branch when no save is running.
    ...
    /// Rebuilds an entry straight from a save, bypassing the copy-on-write shadow (nothing can be
    /// mid-save during a load). `cells` must be in ascending block-index order and match `authored`.
...
/// Builds the chunk the player actually sees: procedural generation, then every authored cell replayed
/// on top, then a flag recompute (replaying ids invalidates the flags the generator derived).
///
/// This is the ONLY way a `mod_store` entry becomes a `Chunk`; the store holds no block data of its own.
...
///
/// The halo both passes read comes from procedural generation (`resolveBaseFoundation()`) or from
/// single-block ancestor lookups, never from a neighbor's `mod_store` entry, so this cannot recurse
/// back into `materializeChunk()`.
...
///
/// Purely procedural: modifications are NOT applied here. Use `materializeChunk()` for the chunk the
...
/// (so an entry may live in a neighboring chunk; `get()` resolves that). A worklist item recomputes
/// its 3x3 of cells, and each of those reads its own 3x3, so the same ~25 cells get fetched well over
/// a hundred times through `getBlockAt()` (a `SimBuffer` probe, then a chunk-cache probe, then possibly
/// a `mod_store` hash lookup or a full chunk generation). The window collapses that to <= 25 fetches.
///
/// Correctness: the drain mutates blocks as it goes, so every write must be mirrored here.
/// `drop()` after writing a cell's flags, `reset()` after any block is cleared.
const BlockWindow = struct {
    ...
    /// Reads the block at (`cx`, `cy`), which may sit outside `coord` and is then resolved into the
    /// neighboring chunk (clamped to `coord` at the world edge, as the cascade has always done).
    ...
    while (flag_worklist.pop()) |item| {
        ...
        while (dy <= 1) : (dy += 1) {
            ...
            while (dx <= 1) : (dx += 1) {
                ...
                // Cascade: a block whose declared neighbors (`Sprite.supports()`, built from the rule
                // table in types/sprite.zig) are no longer there cannot rest here, so it breaks.
                var broken = false;
                ...
                // Only the materialized caches are patched: flags are derived state, so `mod_store` does not
                // store them and `refreshDerivedFlags()` rebuilds them from the replayed ids on the next materialization.
...
///
/// `mod_store` is rebuilt here too, but on `memory.main_allocator` rather than the arena: it frees
/// entries and recycles cell blocks as the player edits, which an arena cannot do. Its `init()` releases
/// the previous world's allocations itself.
...
test "ModEntry: cells stay indexable by block index regardless of insertion order" {
    ...
    // Insert scrambled (a stride coprime with 256 hits every index exactly once) so every insert
    // lands in the middle of the packed array and exercises the shift.
    var n: u32 = 0;
