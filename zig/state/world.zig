...
/// It is cached rather than derived only because the render passes read it per frame,
/// and it does not live in `memory.game`, so anything that rebuilds the world has to restate it
/// (`startup.resetAfterStart()`, `save.finalizeLoad()`).
