...
pub fn pushLayer(parent_id: Sprite, coord: Coordinate, bx: u4, by: u4) void {
    ...
    if (depth >= path_start_depth) {
        ...
        // A future ascent (`popLayer()`) can re-descend a depth whose cell already holds a stale value,
        // so writes CLEAR the 3-bit cell before setting it rather than OR-ing into it. On the forward-only
        // path this is identical (the bits were already zero), but it stops re-descent from corrupting them.
