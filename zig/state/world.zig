...
pub fn clearCaches(comptime clear_ancestors: bool) void {
    ...
    // Debug only, and not for want of caution: a structure placement is a pure function of its cell and
    // the structure seed, which the per-entry seed check already invalidates on, so a reseed or a teleport
    // has nothing to drop here. What the seed does NOT cover is a debug slider retuning the terrain the
    // placement was gated against (`debug_ui.sliders`, `regen`), which moves the ground under a cached
    // verdict without changing a single seed. Release drops its terrain caches on the seed for the same
    // reason (see `procedural.terrainGeneration()`); debug just has more ways for terrain to change.
    if (dw.is_debug) {
