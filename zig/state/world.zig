...
const CHUNK_CACHE_SIZE: usize = blk: {
    ...
    // p-side border, in chunks, of the render/lighting window
    // (see chunk.zig and lighting.CHUNK_MARGIN): margin chunks each side for light bleed,
    // plus 1 for the floor-alignment straddle
    // this all must track CHUNK_MARGIN so widening light range grows the cache.
