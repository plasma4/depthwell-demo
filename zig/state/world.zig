...
pub const QuadCache = struct {
    ...
    /// so an entry only stays true while that depth keeps the same window.
...
pub fn computeLayer(coord: Coordinate, bx: u4, by: u4, anchor: LayerAnchor) LayerTransition {
    ...
    defer {
        ...
        // Step 8 generates under the frame this just chose, which memoizes seeds at `new_depth`.
        // Nothing at D reads them, but a LATER descent to the same depth picks a different window
        // and would read them as its own. Once per descent, and never on a preview install.
        quad_cache.clearSeedCache();
    }
