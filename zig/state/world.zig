    ...
    while (flag_worklist.pop()) |item| {
        ...
        while (dy <= 1) : (dy += 1) {
            ...
            while (dx <= 1) : (dx += 1) {
                ...
                // Most cells the cascade sweeps are unaffected; skipping the identical rewrite avoids three
                // store lookups and, in `mod_store`, a needless save-shadow copy of the whole chunk.
