...
///
/// Lives on `memory.general_allocator`, NOT the world arena: entries are freed and their cell blocks
...
///
/// `mod_store` is rebuilt here too, but on `memory.general_allocator` rather than the arena: it frees
...
pub fn initArenaAllocatedStructures() void {
    ...
    mod_store.init(memory.general_allocator);
    quad_cache.reset();
}
