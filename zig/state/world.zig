...
pub fn generateBaseChunk(chunk: *Chunk, coord: Coordinate) void {
    ...
    stampDecorations(chunk, cx, cy);

    resetEmptyEdgeFlags(chunk);
}

/// Fills still-empty cells from the position-hashed decoration tier (`dw.decorations`).
///
/// Runs after `addDecorations()`, which owns the same cells but decides them from a per-chunk RNG stream.
/// Reads no edge flags: a decoration probes terrain itself, so it neither depends on nor perturbs them.
fn stampDecorations(chunk: *Chunk, cx: u64, cy: u64) void {
    ...
    for (0..CHUNK_SIZE) |block_y| {
        for (0..CHUNK_SIZE) |block_x| {
            ...
            if (dw.decorations.resolve(wx, wy, seed)) |sprite| block.id = sprite;
        }
    }
}
