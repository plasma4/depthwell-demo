...
/// an edit made after the depth was left is local to that depth and must not travel deeper.
...
/// so descending anywhere else would renumber every suffix at that depth and orphan every deeper `mod_store` key.
