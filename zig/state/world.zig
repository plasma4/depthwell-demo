...
/// Those frames do alias, and that is fine: a sweep visits each chunk once, so an alias costs nothing
/// WITHIN a frame. It only stops the NEXT frame from reusing the chunks, which it could not do anyway,
/// since an ascent moves and grows the window every frame.
/// Sizing the tile for it would cost tens of MiB of static slots to save nothing.
...
pub const ModifyBlockTypeResult = enum {
    ...
    collapsed,
    /// Normal-play safety rejected a solid placement before any world state changed.
    rejected_softlock,
};
...
pub fn modifyBlockType(
    ...
    const has_solid = plan.first.sprite.isSolid() or
        (if (plan.second) |second| second.sprite.isSolid() else false);

    if (!dw.inventory.isInCreative() and has_solid) {
