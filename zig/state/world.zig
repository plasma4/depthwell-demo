...
pub const DepthCoordinate = struct {
    ...
    pub inline fn getParent(self: @This()) @This() {
        ...
        // (`HORIZON_DEPTH * ZOOM_LOG2` is the full suffix-lane bit width; shift the top cell to just below it.)
...
pub const QuadCache = struct {
    ...
    /// alias the same slot (up to `HORIZON_DEPTH + 1` depths are live at once), and must be a power of
    /// two so the modulo is a cheap mask. Persisted verbatim in `save.zig`, so changing it breaks saves.
