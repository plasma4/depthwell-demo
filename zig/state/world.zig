...
const FOUNDATION_CACHE_TILE_W = 128;
const FOUNDATION_CACHE_TILE_H = 64;
...
pub fn modifyBlockType(coord: Coordinate, bx: u4, by: u4, new_sprite: Sprite, prev_block: Block) bool {
    // A kind that spans two cells brings its other half with it: the halves pin each other through
    // `requires`, so placing one alone only ever means the cascade deletes it again. Creative-only in
    // practice, since a pair drops as a single item rather than as its halves.
    //
    // BOTH halves are written before ANY of them is validated. Validating in between would judge a half
    // pair: the cascade would find the partner missing and clear the cell that was just placed.
    ...
    // One pass, from the cell the caller named: the cascade walks outward from it, so it reaches every
    // half, and it breaks the pair as a unit if the placement was invalid for any other reason.
