...
pub const AscentStep = struct {
    ...
    /// and re-deriving them from wherever the player has since wandered can land on different ones.
...
/// so it is a CACHE and is never saved (see `QuadCache.getMaterials()`).
...
/// so it can generate the new depth's chunks while the committed world still sits at the old depth.
...
pub fn computeLayer(coord: Coordinate, bx: u4, by: u4, anchor: LayerAnchor) LayerTransition {
    ...
    // The world scales but the player does not. player_pos is the player's CENTER, while the point
    // that must stay on the same surface is the FEET, PLAYER_FEET_OFFSET below the center.
    ...
    // Past the horizon the suffix is full, so a descent must REBASE. Two words for the rest of this
    // function:
    //
    ...
    // Step 8: the horizon. H is HORIZON_DEPTH shallower than the depth being entered, so it moved
    // one depth deeper too, and its window is one refinement step from the window before it.
    const horizon_depth = new_depth - dw.HORIZON_DEPTH;
    if (horizon_depth >= STARTING_ZOOM_TIMES) {
