...
/// out by up to `ZOOM_FACTOR` for the length of the transition, so it draws up to 16 times the chunks.
/// Provisioning for that would cost more than 100 MiB of static slots to save nothing,
/// since almost every chunk it asks for is one the cache has never seen.
...
pub const ChunkCache = struct {
    ...
    /// Claims the `Coordinate`'s slot, evicting whatever held it, and returns the index.
    /// The caller must fill `chunks[index]` before anything reads it.
