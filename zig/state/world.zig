...
pub const ModEntry = struct {
    ...
    /// The markers are a display hint only (see `Block.descendant_mods`), so a stale one costs nothing.
    ///
    /// Always zero in `legacy_store`: a frozen value says nothing about the depths below it.
...
fn materializeChunkInner(chunk: *Chunk, key: DepthCoordinate, comptime view: StoreView) void {
    ...
    if (entry) |e| {
        e.applyTo(chunk);
        // Only meaningful while looking down at a deeper depth; at the frontier the markers left
        // behind by an earlier ascent describe modifications that ARE this layer.
        if (view == .live and isAboveFrontier()) {
