...
/// Top speed is `PLAYER_BASE_SPEED * (1 - DECAY_RATE_ACCEL_X) / DECAY_RATE_ACCEL_X`.
...
/// If `abs(y velocity) < HALVE_JUMP_RANGE` and the jump key is held, then gravity's effect will be multiplied by 60%.
...
pub fn move(logic_speed: f64) void {
    ...
    const up_key_pressed = KeyBits.isSet(KeyBits.up, game.keys_pressed_mask);
    const up_key_held = KeyBits.isSet(KeyBits.up, game.keys_held_mask);

    const jump_requested =
        up_key_pressed or jump_leniency_frames > 0;

    const can_ground_jump =
        coyote_frames > 0;

    const can_air_jump =
        jumps_left < MAX_JUMPS and jumps_left > 0;
    ...
    if (isGhost()) {
        ...
        // Gravity strength by phase of the arc:
        ...
        //   falling under 3.0, key up   1.25  a snappier drop back to the ground
        ...
        // so it goes weak near the top; this one keeps the same bite all the way down to 0.2.
        // The guard stops it from pushing a rising player into a fall.
        const LINEAR_Y_DECAY = 0.2;
    ...
    if (jumps_left == 0 and up_key_pressed and !is_grounded) {
