...
pub fn escapeSolid() void {
    ...
    while (r <= MAX_ESCAPE_BLOCKS) : (r += 1) {
        // rows run top to bottom, so an equally near cell above wins
        var dy: i64 = -r;
        while (dy <= r) : (dy += 1) {
            // columns run outward from the center,
            // so the player never slides sideways past a cell that was free directly above them!
            var k: i64 = 0;
            while (k <= 2 * r) : (k += 1) {
