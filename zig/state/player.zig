...
pub fn move(logic_speed: f64) void {
    ...
    emitMovementDust(is_grounded and !was_grounded, jumped_this_frame and was_grounded, impact_velocity);
}
...
// So the puff flies out, coasts, and settles, with no per-frame drag term.
...
// ---- Dust color. Every knob for it is here. ----
...
pub var DUST_LIGHTNESS_MAX: f32 = 1.08;
/// Chroma jitter, as a multiplier on the tinted chroma.
pub var DUST_CHROMA_MIN: f32 = 0.70;
pub var DUST_CHROMA_MAX: f32 = 1.25;
/// Opacity band a puff picks from, before the pool's own fade curve.
pub var DUST_ALPHA_MIN: f32 = 0.28;
...
///
/// `.none` past the world edge, which the caller then reads as plain white dust.
/// The point must be inside or next to the player hitbox, so its chunk is always resident
/// and this generates nothing.
fn blockSpriteAt(point: Vec2i) Sprite {
    ...
    // `@mod` against a positive divisor is never negative, so both indices land inside the chunk.
...
///
/// One sample per puff, at the puff's OWN x, is what mixes two floor materials by area for free.
/// A player half on green stone and half on plain stone throws each color in that ratio,
/// with no coverage arithmetic and no bias toward whichever block the center happens to be over.
///
/// Precondition: `foot_point` is ON the bottom edge of the hitbox, so the row below it is the floor.
inline fn floorUnder(foot_point: Vec2i) Sprite {
    return blockSpriteAt(foot_point + Vec2i{ 0, 1 });
}

/// The LCHA one puff draws at, taken from the block it was raised off.
///
/// A block gives up its two most common atlas colors, so a vein of one ore inside another
/// throws dust in both. `DUST_TINT` then washes that color toward white.
/// In OKLCH washing toward white is a lightness toward 1 with the chroma going to 0,
/// which keeps the block's hue and drops its saturation, unlike a straight blend with white.
fn dustLcha(ground: Sprite) Vec4f32 {
