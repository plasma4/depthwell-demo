...
pub fn isColliding(px: i64, py: i64) bool {
    ...
    for (corners) |c| {
        ...
        // Per-block rather than per-chunk: `getChunk()` hands back a chunk BY VALUE, and this runs
        // up to four times per CCD step, so the copies dominate a fall.
        if (world.getBlockAt(target_coord, lx, ly, game.depth).isSolid()) return true;
    }
    return false;
}
