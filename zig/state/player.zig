...
/// a damped fall settles at `GRAVITY * (1 - rate) / rate`, so a bigger rate is a slower slide.
...
/// Gravity near the apex with the jump key still down. Under 1 to buy hang time over the top.
pub var GRAVITY_MULT_APEX: f64 = 0.60;
/// Gravity everywhere the arc needs no shaping. Exactly 1, so it must stay the neutral value.
pub const GRAVITY_MULT_BASE: f64 = 1.00;
/// Gravity on a slow fall after the jump key came up. Over 1 for a snappier drop.
pub var GRAVITY_MULT_FALL_SNAP: f64 = 1.20;
/// Gravity while still rising after the jump key came up. The main brake that ends a short jump.
pub var GRAVITY_MULT_RISE_CUT: f64 = 1.80;

/// Fall speed above which `GRAVITY_MULT_FALL_SNAP` stops applying, in world pixels per 60 FPS frame.
...
///
/// `moveSwept()` tests the ground once, after both axes have moved, so the tick that walks off a
/// ledge already reads as airborne. The older axis-at-a-time order tested it mid-move and so gave
/// one extra forgiving tick; this value carries that tick instead.
...
/// Whether the player slid down a wall on this tick. Read by the dust emitter.
