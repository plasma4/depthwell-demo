...
pub fn permitsPlacement(cells: []const PendingPlacement) bool {
    ...
    return reachesEscapeBoundary(&overlay, canExcavateOffset);
}
...
pub fn escapeSolid() bool {
    ...
    if (escape_seen[escapeIndex(0, 0)]) return false;
...
/// Searches outward from the player until one path reaches the bounded escape edge.
///
/// A placement needs only one escape witness, so this depth-first walk is usually a short path.
/// `escapeSolid()` instead uses `floodOutside()` because it must find a safe correction landing.
fn reachesEscapeBoundary(context: anytype, comptime can_enter: anytype) bool {
    @memset(&escape_seen, false);
    @memset(&escape_checked, false);
    var write: usize = 0;
