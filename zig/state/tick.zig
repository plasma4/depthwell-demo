...
/// `iterations` is a catch-up count after a stall, and `logic_speed` is how many 60 FPS frames
/// ONE iteration stands for (`logicLoop()` in `src/main.ts` raises it when the tick rate drops).
