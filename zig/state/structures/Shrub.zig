...
//! LR
//!
//! Previously generated in `procedural.addDecorations()`, which could only start a pair on an EVEN column,
//! and never across a chunk border (the decor pass writes one chunk at a time and has no view of the next).
//! As a structure it is position-hashed from world coordinates, so it is free of both limits,
//! and the left/right halves are pinned to each other by the `requires` rules in `types/sprite.zig`.
