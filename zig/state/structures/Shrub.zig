...
//! As a structure it is position-hashed from world coordinates, so it is free of both limits, and the
//! left/right halves are pinned to each other by the `requires` rules in `types/sprite.zig`.
