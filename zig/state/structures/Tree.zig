...
//!        T
//!
//! Placement is gated on BASE terrain only (pre-ore, pre-structure) by the declarative `constraints` list;
//! `structures.zig` evaluates it once per grid cell and memoizes the verdict, so the terrain noise is never
//! re-sampled for the other 34 cells of the footprint.
...
pub const target_chance: f64 = 0.25;
