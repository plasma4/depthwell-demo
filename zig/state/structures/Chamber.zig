...
///
/// The jittered box is pulled DOWN until the row below it rests on the terrain surface. Without seating, the
/// box keeps whatever y the hash happened to pick, and would only look grounded by coincidence.
///
/// Flatness is checked HERE rather than as a `.level` constraint, because constraints only run after bounds
/// exist -- by which point the scan has already been paid for. Nearly every candidate sits on uneven ground,
/// so folding the test into the scan and bailing the moment two columns disagree is what keeps the cost down:
/// the common case dies after two columns instead of profiling all `size_x` of them.
///
/// With nothing under the room to bridge a dip, flat means exactly flat. A rise would bury the base row and a
/// drop would leave the room floating over a gap, so every column's surface must land on the same row.
