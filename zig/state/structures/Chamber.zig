...
/// The ground line: the first row below the ROOM, which is where the terrain surface has to land.
const ground_row: structures.Edge = .{ .at = .end, .off = -PLINTH_ROWS };
/// One row past `ground_row`, for the region rules below.
const below_ground_row: structures.Edge = .{ .at = .end, .off = -PLINTH_ROWS + 1 };
...
pub const constraints = [_]structures.Constraint{
    ...
    // above the ground line (nothing buries the room), while a dip of up to `PLINTH_ROWS` is fine, since the
    // plinth fills it in.
    .{ .level = .{
        .row = ground_row,
        .max_slope = PLINTH_ROWS,
        .max_rise = 0,
        .max_drop = PLINTH_ROWS,
    } },
};

/// Anchors the candidate, then SEATS it: the jittered box is pulled down until the room's base row rests on
/// the HIGHEST ground under the footprint. Without seating, the box keeps whatever y the hash happened to
/// pick, and would only look grounded when that y coincidentally matched the terrain.
///
/// Seating on the highest column rather than (say) the center is what makes `max_rise = 0` achievable: by
/// construction no column's ground can then rise above the room, so every remaining column dips, and dips are
/// exactly what the plinth is for. Seating on the center instead would demand the center happen to be the
...
pub fn getBounds(state: *HashState, cx: i32, cy: i32) Rect {
    ...
    const room_base = bounds.y_end - PLINTH_ROWS;

    var highest: ?i32 = null;
    var x = bounds.x_start;
    while (x < bounds.x_end) : (x += 1) {
        const surface = structures.surfaceY(x, room_base, room_base + SNAP_DEPTH) orelse continue;
    ...
    const seat = (highest orelse return bounds) - room_base;
    bounds.y_start += seat;
    bounds.y_end += seat;
    return bounds;
}
