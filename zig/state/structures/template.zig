...
//!
//! A structure draws its footprint as a multiline string and hands over a legend mapping each character
//! to what it places. The whole thing is parsed and validated at compile time, so a typo in the art is a
//! build error with a row/column, not a silent wrong block.
//!
//! This is for shapes where placement is intentional (a room, a vestibule, a chest alcove). Organic or
//! size-varying shapes (a geode's rolled disc) stay as math in `generate()`; the two coexist.
//!
//! Conventions:
//! - A space is GUARANTEED AIR by default (`.none`, actively clearing terrain), because that is the most
//!   intuitive reading of blank space in the art. A legend may override `' '` if it wants something else.
//! - Every other character must appear in the legend, or the build fails naming the character and where.
//! - Rows must all be the same length. A ragged row is almost always an accident, so it is a build error
//!   rather than being silently padded.
//! - `resolve()` returning null means "not mine": the structure's own fallback (or the terrain) shows through.
