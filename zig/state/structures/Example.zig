...
/// Ground odds are only estimated heuristically so see `debug/audit.zig`.
...
///
/// Only worth raising for a heavily gated structure. The hut needs flat ground, which is rare,
...
/// STAGE 3, SEAT: stands the hut on the ground.
///
...
/// STAGE 4, GATE. Every rule here is ANDed, and runs only once the box is final (post-seating).
...
// STAGE 2, ANCHOR: there is deliberately no getBounds() here.
//
// The hut is a plain size_x-by-size_y rectangle, so structures.zig's default jitter already does the job,
// and writing `return jitter(...)` by hand would be pure boilerplate. Only write getBounds() when the box
// is something the defaults cannot express, which in practice means a size rolled at runtime
// (Geode.zig rolls a radius, Ancient.zig rolls two).
//
// Whatever you write, route it through jitter(). It anchors the footprint uniformly ANYWHERE in the cell,
// overhang included. Drawing an origin from [0, spawn_area - w) instead would seem more natural but blanks
// out a band along every cell edge, which makes the spawn lattice visible as a regular grid in the world.
