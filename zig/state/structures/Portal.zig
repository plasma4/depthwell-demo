...
/// Deriving the shell from `inShape()` rather than testing for the footprint's edges keeps the wall
/// exactly one block thick all the way around the chamfer, with no special-casing per corner.
...
pub fn generate(
    ...
    _ = .{ starting_sprite, cx, cy, state, struct_seed };
