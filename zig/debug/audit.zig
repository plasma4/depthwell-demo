//! Debug-only sampler for worldgen odds.
//!
//! `target_chance` is a roll, not a density: terrain rules throw most rolls away and how many is unknowable at
//! compile-time, so the only honest way to tune a structure is to place a lot of world and count. This does
//! that, over a region around the player, and reports what actually landed.
//!
//! Three things get counted:
//! - structures: cells rolled, placements standing on valid terrain, and how many survived a rival
//! - decorations: anchors standing, and how many survived an overlapping anchor
//! - blocks: the sprite mix, which is the `ore odds` question
//!
//! Everything is sampled straight from world coordinates, so it never touches the chunk pipeline or the caches
//! the game is using. It is also the reason `procedural.addDecorations()` is MISSING from the block mix below:
//! that pass draws from a per-chunk RNG stream rather than a position hash, so a single block cannot be asked
//! about in isolation. That is a limitation of the pass, not of this file.
const std = @import("std");
const dw = @import("../root.zig");

const logger = dw.logger;
const memory = dw.memory;
const structures = dw.structures;
const decorations = dw.decorations;

const Sprite = dw.Sprite;

/// Blocks per axis sampled by `runAroundPlayer()`. Big enough that the rarest structure lands often enough to
/// mean something, small enough to stay well under a second.
const DEFAULT_SPAN: i32 = 2048;

/// Distinct sprites tracked in the block mix.
const MAX_TRACKED_SPRITES = 64;

/// Runs the audit on a region centered on the player's base-depth position, and logs the result.
/// Depth is ignored: everything here is a property of the base layer the whole fractal descends from.
pub fn runAroundPlayer() void {
    if (!dw.is_debug) return;

    const coord = memory.game.getPlayerCoord();
    const center_x: i32 = @intCast(@min(coord.suffix[0] *| dw.CHUNK_SIZE, std.math.maxInt(i32)));
    const center_y: i32 = @intCast(@min(coord.suffix[1] *| dw.CHUNK_SIZE, std.math.maxInt(i32)));
    run(center_x - @divTrunc(DEFAULT_SPAN, 2), center_y - @divTrunc(DEFAULT_SPAN, 2), DEFAULT_SPAN);
}

/// Samples the `span` x `span` block region whose corner is (`x0`, `y0`) and logs every tally.
pub fn run(x0: i32, y0: i32, span: i32) void {
    if (!dw.is_debug) return;

    const seed = memory.game.getHashSeed(.structures);
    const blocks: f64 = @floatFromInt(@as(i64, span) * @as(i64, span));
    const per_million = 1_000_000.0 / blocks;

    logger.log(@src(), "=== worldgen audit: {d}x{d} blocks at ({d}, {d}) ===", .{ span, span, x0, y0 });

    auditStructures(x0, y0, span, seed, per_million);
    auditDecorations(x0, y0, span, per_million);
    auditBlocks(x0, y0, span);
}

/// Per structure kind: how many cells rolled a placement the terrain accepted, and how many of those a rival
/// then beat. The gap between the two is what the collision system is costing you.
///
/// `offset` is the mean anchor position within the cell against the uniform expectation. A number that drifts
/// says placements favor one corner of their cell, which would show up as a visible lattice.
fn auditStructures(x0: i32, y0: i32, span: i32, seed: dw.utils.Vec2u, per_million: f64) void {
    logger.log(@src(), "{s:<10} {s:>6} {s:>7} {s:>8} {s:>8} {s:>10} {s:>16}", .{
        "structure", "chance", "cells",  "standing",
        "placed",    "per 1M", "offset (uniform)",
    });

    inline for (0..structures.structures.len) |kind| {
        const S = structures.structures[kind];
        const area: i32 = @intCast(S.spawn_area);

        var cells: u64 = 0;
        var standing: u64 = 0;
        var placed: u64 = 0;
        var sum_ox: f64 = 0;
        var sum_oy: f64 = 0;

        var cy = @divFloor(y0, area);
        while (cy <= @divFloor(y0 + span - 1, area)) : (cy += 1) {
            var cx = @divFloor(x0, area);
            while (cx <= @divFloor(x0 + span - 1, area)) : (cx += 1) {
                cells += 1;
                if (structures.getStructureBounds(kind, cx, cy, seed) == null) continue;
                standing += 1;

                const bounds = structures.acceptedBoundsForAudit(kind, cx, cy, seed) orelse continue;
                placed += 1;
                sum_ox += @floatFromInt(bounds.x_start - cx * area);
                sum_oy += @floatFromInt(bounds.y_start - cy * area);
            }
        }

        const n: f64 = @floatFromInt(placed);
        const uniform: f64 = @as(f64, @floatFromInt(area - 1)) / 2.0;
        logger.log(@src(), "{s:<10} {d:>6.3} {d:>7} {d:>8} {d:>8} {d:>10.1} {d:>6.2}/{d:.2} ({d:.2})", .{
            shortName(@typeName(S)),
            S.target_chance,
            cells,
            standing,
            placed,
            n * per_million,
            if (placed > 0) sum_ox / n else 0,
            if (placed > 0) sum_oy / n else 0,
            uniform,
        });
    }
}

/// Per decoration kind: anchors the terrain accepted, and how many survived an overlapping anchor of the
/// same kind. Decorations have no grid, so density is the only dial and this table IS the dial's readout.
fn auditDecorations(x0: i32, y0: i32, span: i32, per_million: f64) void {
    logger.log(@src(), "{s:<10} {s:>6} {s:>8} {s:>8} {s:>10}", .{ "decor", "chance", "standing", "placed", "per 1M" });

    const seed = memory.game.getHashSeed(.decorations1);
    inline for (0..decorations.decorations.len) |kind| {
        const D = decorations.decorations[kind];

        var standing: u64 = 0;
        var placed: u64 = 0;
        var wy = y0;
        while (wy < y0 + span) : (wy += 1) {
            var wx = x0;
            while (wx < x0 + span) : (wx += 1) {
                if (!decorations.standsForAudit(kind, wx, wy, seed)) continue;
                standing += 1;
                if (decorations.anchoredForAudit(kind, wx, wy, seed)) placed += 1;
            }
        }

        logger.log(@src(), "{s:<10} {d:>6.3} {d:>8} {d:>8} {d:>10.1}", .{
            shortName(@typeName(D)),
            D.chance,
            standing,
            placed,
            @as(f64, @floatFromInt(placed)) * per_million,
        });
    }
}

/// The sprite mix of the region, most common first. This is the `block odds` readout: what fraction of the
/// world is stone, air, each ore, each gem.
fn auditBlocks(x0: i32, y0: i32, span: i32) void {
    var ids: [MAX_TRACKED_SPRITES]Sprite = @splat(.none);
    var counts: [MAX_TRACKED_SPRITES]u64 = @splat(0);
    var tracked: usize = 0;
    var total: u64 = 0;
    var dropped: u64 = 0;

    const decor_seed = memory.game.getHashSeed(.decorations1);

    var wy = y0;
    while (wy < y0 + span) : (wy += 1) {
        var wx = x0;
        while (wx < x0 + span) : (wx += 1) {
            if (wx < 0 or wy < 0) continue;

            var sprite = dw.world.sampleBaseFoundation(@bitCast(wx), @bitCast(wy));
            if (sprite.isEmpty()) {
                if (decorations.resolve(@bitCast(wx), @bitCast(wy), decor_seed)) |d| sprite = d;
            }
            total += 1;

            const slot = for (ids[0..tracked], 0..) |id, i| {
                if (id == sprite) break i;
            } else blk: {
                if (tracked == MAX_TRACKED_SPRITES) {
                    dropped += 1;
                    break :blk null;
                }
                ids[tracked] = sprite;
                tracked += 1;
                break :blk tracked - 1;
            };
            if (slot) |i| counts[i] += 1;
        }
    }
    if (total == 0) return;

    // selection sort: `tracked` is tiny and this keeps the debug path allocation-free
    for (0..tracked) |i| {
        var best = i;
        for (i + 1..tracked) |j| {
            if (counts[j] > counts[best]) best = j;
        }
        std.mem.swap(u64, &counts[i], &counts[best]);
        std.mem.swap(Sprite, &ids[i], &ids[best]);
    }

    logger.log(@src(), "{s:<20} {s:>10} {s:>8}", .{ "sprite", "count", "share" });
    for (0..tracked) |i| {
        logger.log(@src(), "{s:<20} {d:>10} {d:>7.3}%", .{
            @tagName(ids[i]),
            counts[i],
            100.0 * @as(f64, @floatFromInt(counts[i])) / @as(f64, @floatFromInt(total)),
        });
    }
    if (dropped > 0) logger.err(@src(), "block mix truncated: more than {d} distinct sprites", .{MAX_TRACKED_SPRITES});
}

/// Trims `state.structures.Chamber` down to `Chamber`, so the tables line up.
inline fn shortName(comptime full: []const u8) []const u8 {
    comptime {
        var start: usize = 0;
        for (full, 0..) |c, i| {
            if (c == '.') start = i + 1;
        }
        const trimmed = full[start..].*;
        return &trimmed;
    }
}
