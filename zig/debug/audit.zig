...
pub fn sampleWorldAroundPlayer() void {
    ...
    // `suffix` is measured in chunks, while the audit span is measured in blocks. Compare in the
    // same units or the normal starting position (only a few chunks from the origin) looks unsafe.
    const edge_margin_chunks = @divCeil(DEFAULT_SPAN, dw.CHUNK_SIZE);
