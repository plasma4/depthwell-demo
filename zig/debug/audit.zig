...
//! the game is using. Column features (vines) are the one thing MISSING from the block mix: a chain is walked
//! down a column rather than resolved per block, so a single block cannot be asked about in isolation.
