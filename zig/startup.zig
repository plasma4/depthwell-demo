...
fn resetAfterStart() void {
    ...
    memory.game = .{};
    // `max_possible_suffix` is derived from the depth but does NOT live in `memory.game`, so the line
    // above does not touch it: a run that descended leaves the previous depth's world edge behind, and
    // everything bounded by it (the render window, the debug teleport) then reaches past the new world.
