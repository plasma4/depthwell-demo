...
export class SaveManager {
    ...
    */
    ...
    private applyBytesSync(bytes: Uint8Array, sourceName: string): boolean {
        // Copy bytes into WASM heap and parse synchronously. Failures are pure here: the caller
        // decides when a whole load has run out of fallbacks, so a single retry never resets or
        // alerts on its own (see reportImportFailure()).
        const ptr = Number(
     ...
     * then a single reset to a fresh game. Call exactly once per failed load, never per attempt.
     */
    private reportImportFailure(): void {
        ...
        console.warn(`Save import failed: ${label}`);
        this.engine.start();
        alert(`Couldn't load your save — ${label}. Starting a new game.`);
    }

    /**
     * Loads the committed save, falling back emergency -> MAIN -> BAK.
     * Returns true when the game is left in a running state — either a save applied, or every source
     * failed and reportImportFailure() reset to a fresh game (alerting once). Returns false only when
     * no save existed at all, so the caller should start a fresh game itself.
     */
    public async load(): Promise<boolean> {
        ...
        // A save existed but no source could be applied: report once and reset to a fresh game.
        if (attempted) {
