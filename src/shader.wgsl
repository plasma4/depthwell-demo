...
/*
    ...
    The whole water field is periodic over WATER_PERIOD pixels (256 chunks, the same window
    zig/render/chunk.zig reduces grid_origin into), which is what keeps the coordinates handed to
    these functions inside f32 precision no matter how far out in the world the player is.

    Every layer is therefore written as an integer wave vector: whole cycles across WATER_PERIOD on
    EACH axis, not a unit direction times a wavelength. A unit direction leaves both axes with a
    fractional cycle count, so the field does not close on itself and the wrap boundary shows a seam.
    Time is likewise given in cycles per second, so an animated layer returns to its own phase.
*/
...
//   .y a broad swell in [0, 1] the body uses for its lightness gradient, free of extra trig here.
//
// The swell is the reason the field stops reading as a lattice: it drifts slowly across roughly six
// chunks and gates the two caustic directions AGAINST each other, so bands strengthen and fade in
// patches and the two families weave instead of overlaying into a regular grid.
fn water_effect(coord: vec2f, t: f32) -> vec2f {
    ...
    // Crests of A only, sharpened to a glint. Free: it reuses A rather than sampling anything new.
    let s2 = band_a * band_a;
    let s4 = s2 * s2;
    let sparkle = s4 * s4 * 0.5;
    ...
    // Layer E: A's perpendicular, coarser. Where swell_bias favours A this fades out, and vice versa.
    let e1 = water_wave(world2, vec2f(396.0, 848.0), -0.09, t);
    let e2 = water_wave(world2, vec2f(533.0, 1142.0), 0.13, t);
    let temp_e = e1 * e2;
    let band_e = temp_e * temp_e * 0.30;

    // Fine warp for the curvy streak, kept separate from the coarse one so it can be much tighter.
