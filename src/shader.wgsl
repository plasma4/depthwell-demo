...
fn unpack_tile(data: TileData) -> UnpackedTile {
    ...
    // The three light channels stay packed here. The fragment stage reads them per PIXEL out of
    // `tiles` instead, blending the four tiles nearest it; see tile_light().
...
//
// The tint travels as a vector, not as a (chroma, hue) pair. A hue is an angle, and a blend of two
// angles walks the hue circle through colors that no light in the scene ever had.
...
//
// zig/render/lighting.zig solves one light value per block, so reading a tile's own value gives a
// 16px mosaic: hard steps in lightness, and a hard step in chroma where a lamp's color takes over.
// Sampling on tile CENTERS instead turns both into a gradient. Costs three extra tile loads.
