...
// One instance is one background pixel: a quad on the world block grid, whose color is the noise
// sampled once at the cell center and held flat over the quad. This replaces a full-screen triangle
// that ran the same noise for every canvas pixel, so the cost follows the cell count instead of the
