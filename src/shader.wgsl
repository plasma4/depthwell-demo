...
//
// The clamp is a safety net, not a visual feature: it only fires outside the tile grid, and Zig pads
// the grid by lighting.CHUNK_MARGIN chunks on every side, which is far more than the one tile a
// visible pixel can reach past itself. See LIGHT_FILTER_REACH_TILES in zig/render/chunk.zig, which
// proves that margin at comptime. Clamping (rather than wrapping) keeps a bug subtle instead of
// smearing the far side of the screen across this one.
