...
// One instance is one background pixel: a quad on the world pixel grid, whose color is the noise
// sampled once at the cell center and held flat over the quad. This replaces a full-screen triangle
// that ran the same noise for every canvas pixel, so the cost follows the camera zoom instead of the
// resolution. The cell size and the row length come from publishBackgroundGrid() in chunk.zig.
@vertex
fn vs_background(
    @builtin(vertex_index) vertex_index: u32,
    @builtin(instance_index) instance_index: u32
    ...
    // and not to the screen. A cell corner is a whole multiple of the cell size and stays well inside
    // 2^24, so two neighbours agree on their shared edge to the bit and cannot leave a seam.
