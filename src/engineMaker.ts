    ...
    const bgPipeline = device.createRenderPipeline({
        ...
        fragment: {
            ...
            targets: [
                {
                    format: format,
                    // fs_background returns premultiplied colour (rgb * opacity, opacity), so the
                    // background cross-dissolves during a portal descent, where the D+1 background is
                    // drawn over D's. At the usual opacity of 1 this is identical to writing opaquely.
                    blend: {
                        color: {
                            srcFactor: "one",
                            dstFactor: "one-minus-src-alpha",
